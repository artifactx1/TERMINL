...
export default function WenLambo(){
  ...
  return <div ref={shell} className={`${s.shell} ${playing?s.inRace:''}`}>
    ...
    {mode==='menu'?<main className={s.menu}>
      ...
      <div className={s.raceBar}><b>{mode==='practice'?`PRACTICE / BOT: ${botProfile?.name||'RIVAL'}`:seat?.slot===-1?'LIVE SPECTATOR':'ONLINE / SERVER AUTHORITATIVE'}</b><span>{TRACKS[view?.track]?.name} · RACE {(view?.trackIndex||0)+1}/{view?.tracks.length}</span><span>{mode==='online'?`${connection.toUpperCase()} · ${rtt}ms`:'NO MATCH REWARDS'}</span></div>
