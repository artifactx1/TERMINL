...
import {composeRaceInput,decodeRaceSteering,raceKeyboardInput,RACE_KEYS as KEYS} from '../../lib/arcade/race-input.mjs';
import {createPracticeRival} from '../../lib/arcade/race-bot.mjs';
import {rivalTelemetry,cupStanding} from '../../lib/arcade/race-telemetry.mjs';
import {createChallengeRace} from '../../lib/arcade/bot-challenge.mjs';
import {useBotChallenge} from './use-bot-challenge';
import {BotChallengePanel,BotVictory} from './BotChallengePanel';
...
export default function WenLambo(){
  const official=useBotChallenge();
  ...
  const current=useRef(null);current.current={mode,paused,prefs,tutorial,lesson,mapOpen,settings,official};
  ...
  useEffect(()=>{
    ...
    const frame=now=>{
      ...
      const el=canvas.current;if(state&&el){
        ...
        el.dataset.steeringTarget=String(decodeRaceSteering(input.current));el.dataset.rival=bot.current?.name||'';el.dataset.rivalSeed=String(bot.current?.seed??'');
  ...
  const practice=(teach,cup=!teach)=>{
    ...
    bot.current=createPracticeRival({playerVehicle:vehicle,previous:bot.current,teaching:teach});setBotProfile(bot.current);
    game.current=createRace({vehicles:[vehicle,bot.current.vehicle],track,cup});setView(game.current);setMode('practice');audio.current?.setPaused(false);audio.current?.unlock();
  };
  const startOfficial=async()=>{
    const attempt=await official.start();if(!attempt)return;
    client.current?.dispose();client.current=null;neutral();setNotice('');setRoom(null);setSeat(null);setResult(null);setPaused(false);setTutorial(false);setLesson(0);
    bot.current=attempt.challenge.bot;setBotProfile(bot.current);
    game.current=createChallengeRace(attempt.challenge);setView(game.current);setMode('practice');audio.current?.setPaused(false);audio.current?.unlock();
  };
  ...
  const leave=()=>{official.cancel();client.current?.dispose();client.current=null;game.current=null;neutral();setMode('menu');setRoom(null);setView(null);setSeat(null);setResult(null);setNotice('');setPaused(false);setTutorial(false);};
  ...
  const mySlot=Math.max(0,seat?.slot??0),cupMode=(view?.tracks?.length||1)>1,cup=cupMode&&standings?cupStanding(standings,mySlot):null;
  const lastTimes=view?.raceResults.at(-1)?.times,lastRace=lastTimes?lastTimes[mySlot]===null?'DID NOT FINISH':lastTimes[1-mySlot]===null||lastTimes[mySlot]<lastTimes[1-mySlot]?'YOU WON':lastTimes[mySlot]===lastTimes[1-mySlot]?'DEAD HEAT':'YOU FINISHED 2ND':null;
  ...
  return <div ref={shell} className={`${s.shell} ${playing?s.inRace:''}`}>
    <header className={s.header}><Link href='/os'><b>TERMINL</b><span>ARCADE</span></Link><span className={s.preview}>WEN LAMBO / EXOTICS UPDATE</span><div>{mode!=='menu'&&<button onClick={leave}>← LEAVE</button>}<button onClick={()=>{neutral();setSettings(true);if(mode==='practice')setPaused(true);}}>SETTINGS</button><button aria-label='Toggle fullscreen' onClick={()=>{if(document.fullscreenElement)document.exitFullscreen?.().catch(()=>{});else shell.current?.requestFullscreen?.().catch(()=>setNotice('Fullscreen is unavailable.'));}}>⛶</button></div></header>
    ...
    {mode==='menu'?<main className={s.menu}>
      <BotChallengePanel official={official} onStart={startOfficial}/>
      ...
      <div className={s.garage}><section><div className={s.sectionTitle}><b>01 / YOUR GETAWAY</b><span>EVERY RIDE IS FREE</span></div><div className={s.cars}>{Object.values(VEHICLES).map(v=><button key={v.id} aria-pressed={vehicle===v.id} onClick={()=>setVehicle(v.id)}><VehiclePreview vehicle={v.id}/><small>{v.tag}</small><h2>{v.name}</h2><p>{v.description}</p><small className={s.driverLabel}>DRIVER / {v.driver}</small><span className={s.selected}>{vehicle===v.id?'SELECTED ↗':'SELECT +'}</span></button>)}</div></section>
      <aside className={s.playPanel}><span className={s.eyebrow}>THE AFTER-HOURS CUP</span><h2>California.<br/>Full send.</h2><p>Six distinct courses. Three laps each. Drift through turns, release to bank boost, then spend it on the straight.</p><button className={s.primary} onClick={()=>practice(false)}>SIX-COURSE CUP →</button><button className={s.secondary} onClick={()=>practice(false,false)}>QUICK RACE →</button><button className={s.secondary} onClick={()=>practice(true)}>LEARN TO DRIVE ↗</button><div className={s.divider}>REAL ONLINE PvP / 2-DRIVER CUP</div><label>YOUR CALLSIGN<input maxLength={20} value={name} onChange={e=>setName(e.target.value.replace(/[^a-zA-Z0-9 _-]/g,''))}/></label><button className={s.primary} onClick={()=>online('create')}>CREATE RACE +</button><label>RACE INVITATION<input value={invite} onChange={e=>setInvite(e.target.value)} placeholder='Paste a full race invitation'/></label><div className={s.two}><button className={s.secondary} onClick={()=>online('join')}>JOIN RACE →</button><button className={s.secondary} onClick={()=>online('watch')}>SPECTATE</button></div>{resume&&<button onClick={()=>online('resume')}>RECONNECT TO {resume.code}</button>}<small>A different bot, car and driving style each practice run. Real players online. No cash, tokens, or NFT prizes.</small></aside></div>
      ...
      <footer className={s.footer}><p>Five supercars. One Bike Tyson. Six free rides. Skill-earned boost. All performance classes free. No real car brand affiliation.</p><Link href='/os'>BACK TO THE ARCADE ↗</Link></footer>
      ...
      <div className={s.raceBar}><b>{official.attempt?'OFFICIAL / BARRYBOT':mode==='practice'?`PRACTICE / BOT: ${botProfile?.name||'RIVAL'}`:seat?.slot===-1?'LIVE SPECTATOR':'ONLINE / SERVER AUTHORITATIVE'}</b><span>{TRACKS[view?.track]?.name} · RACE {(view?.trackIndex||0)+1}/{view?.tracks.length}</span><span>{mode==='online'?`${connection.toUpperCase()} · ${rtt}ms`:official.attempt?'VERIFIED PRE-MINT CHALLENGE':'NO MATCH REWARDS'}</span></div>
      <div className={s.hud}><div data-testid='race-position' className={s.position}><small>{seat?.slot===-1?'DRIVER 1':'YOUR POSITION'}</small><strong>{standing?.place===1?'1st':standing?.place===2?'2nd':'—'}<i>/2</i></strong></div>{cup&&<div data-testid='cup-standing' className={s.position}><small>CUP</small><strong>{cup.label}<i>{cup.mine}–{cup.theirs}</i></strong></div>}<div><small>LAP</small><strong>{Math.min((me?.lap||0)+1,3)}<i>/3</i></strong></div><div><small>RACE TIME</small><b>{((view?.raceTicks||0)/60).toFixed(1)}s</b></div><div className={s.boost}><small>BOOST</small><div><i style={{width:`${me?.boost||0}%`}}/></div><span>{me?.drifting?`DRIFT BANK +${Math.floor(me.driftCharge)}`:`${Math.floor(me?.boost||0)}% · RELEASE DRIFT TO BANK`}</span></div><div><small>SPEED</small><strong>{me?.gear===-1?'R ':''}{Math.round((me?.speed||0)*42)}<i>km/h*</i></strong></div>{rival&&<button className={s.mobileRival} data-testid='mobile-rival' aria-label={`${mapOpen?'Hide':'Show'} track map. ${rival.label}. ${rival.direction}. Map closes after four seconds.`} aria-pressed={mapOpen} onClick={()=>setMapOpen(open=>!open)}><small><span aria-hidden='true' style={{display:'inline-block',transform:`rotate(${rival.bearing}rad)`}}>↑</span> RIVAL · {mapOpen?'CLOSE':'MAP'}</small><b>{rival.label.replace('RIVAL ','')}</b></button>}</div>
      <div className={s.arena} ref={arena}><canvas ref={canvas} tabIndex={0} aria-label='WEN LAMBO race. Up accelerates, left and right steer, down brakes and reverses, space drifts, Shift boosts, R recovers.'/>
        ...
        {official.attempt&&done&&<BotVictory official={official} onRetry={startOfficial} onLeave={leave}/>}
        {!official.attempt&&(view?.phase==='raceOver'||done)&&<div className={s.overlay}><span className={s.eyebrow}>{done?mode==='online'?`SERVER-CONFIRMED ${cupMode?'CUP':'RACE'} RESULT`:`PRACTICE ${cupMode?'CUP':'RACE'} COMPLETE`:`CHEQUERED FLAG · RACE ${(view?.trackIndex||0)+1} OF ${view?.tracks.length}`}</span><h2>{done?outcome?.winner===null?'DEAD HEAT.':seat?.slot===-1?`${cupMode?'CUP':'RACE'} COMPLETE.`:outcome?.winner===(seat?.slot??0)?'BAG SECURED.':cupMode?'CUP LOST ON POINTS.':'NEXT ONE IS YOURS.':seat?.slot===-1?'ONE RACE DOWN.':lastRace==='YOU WON'?'RACE WON.':lastRace==='DEAD HEAT'?'DEAD HEAT.':lastRace==='DID NOT FINISH'?'DID NOT FINISH.':'RACE LOST.'}</h2>{cup&&<p className={s.cupLine} data-testid='cup-line'>{seat?.slot===-1?'DRIVER 1 · ':''}CUP {cup.label} · {cup.mine}–{cup.theirs} PTS{lastRace?` · LAST RACE: ${lastRace}`:''}<br/><b>{cup.status}</b></p>}<div className={s.scoreboard}>{view?.players.map((p,i)=><div key={i}><b>{room?.players[i]?.name||(i?botProfile?.name||'PRACTICE BOT':name)}</b><span>{p.points} PTS</span><small>{seconds(view.raceResults.at(-1)?.times[i]??null)}</small></div>)}</div>{done?<><p>{mode==='online'?'Recorded by the server. No financial prizes.':'Practice results stay local.'}</p>{seat?.slot!==-1&&<button className={s.primary} onClick={()=>mode==='practice'?practice(false):client.current?.send({type:'rematch'})}>{mode==='practice'?'RACE ANOTHER CUP →':room?.players[seat?.slot]?.rematch?'WAITING FOR RIVAL…':'VOTE REMATCH →'}</button>}<button onClick={leave}>BACK TO GARAGE</button></>:<p>{view.trackIndex+1<view.tracks.length?`NEXT: ${TRACKS[view.tracks[view.trackIndex+1]].name}`:'FINAL STANDINGS INCOMING'} · {Math.max(0,8-Math.floor(view.phaseTick/60))}s<br/>1st: 10 pts · 2nd: 6 pts · DNF: 0 · Equal finish: 8 each<br/>Cup ties: lower combined race time wins.</p>}</div>}
      </div>
...
function VehiclePreview({vehicle,large=false}){
  const ref=useRef(null);useEffect(()=>{let disposed=false;const paint=()=>{if(disposed||!ref.current)return;const c=ref.current.getContext('2d');c.clearRect(0,0,500,250);drawGarageVehicle(c,vehicle,{x:250,y:240,width:large?480:460,height:225});};paint();preloadRaceArt().then(paint);return ()=>{disposed=true;};},[vehicle,large]);
  return <canvas ref={ref} width={500} height={250} aria-label={vehicle==='bike-tyson'?'Bike Tyson human-bicycle meme racer':`${VEHICLES[vehicle]?.name} arcade roadster with ${VEHICLES[vehicle]?.driver}`}/>;
}
