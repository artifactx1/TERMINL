...
export default function WenLambo(){
  ...
  const rival=view?rivalTelemetry(view,Math.max(0,seat?.slot??0)):null;
  return <div ref={shell} className={`${s.shell} ${playing?s.inRace:''}`}>
    <header className={s.header}><Link href='/os'>▣ TERMINL <span>ARCADE</span></Link><span className={s.preview}>WEN LAMBO / GRAND TOUR v4</span><div>{mode!=='menu'&&<button onClick={leave}>← LEAVE</button>}<button onClick={()=>{neutral();setSettings(true);if(mode==='practice')setPaused(true);}}>SETTINGS</button><button aria-label='Toggle fullscreen' onClick={()=>{if(document.fullscreenElement)document.exitFullscreen?.().catch(()=>{});else shell.current?.requestFullscreen?.().catch(()=>setNotice('Fullscreen is unavailable.'));}}>⛶</button></div></header>
