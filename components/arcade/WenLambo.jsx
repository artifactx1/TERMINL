...
import {createRace,stepRace,raceBotInput,TRACKS,VEHICLES} from '../../lib/arcade/race-sim.mjs';
...
export default function WenLambo(){
  ...
  const neutral=()=>{keyboard.current=0;touch.current=0;pad.current=0;input.current=0;held.current.clear();client.current?.setInput(0);};
  useEffect(()=>{
  ...
  const practice=teach=>{
    ...
    game.current=createRace({vehicles:[vehicle,vehicle==='comet'?'spectre':'comet'],track,cup:!teach});setView(game.current);setMode('practice');audio.current?.setPaused(false);audio.current?.unlock();
  };
  ...
  const online=kind=>{
    ...
    else if(kind==='create')action={type:'create',game:'wen-lambo',rulesVersion:1,name:name.trim()||'ANON',character:vehicle,stage:track};
    else{const parsed=invitation(invite);if(!parsed){setNotice('Paste the complete race invitation, including its token.');return;}action={type:'join',game:'wen-lambo',rulesVersion:1,...parsed,name:name.trim()||'ANON',character:vehicle,spectator:kind==='watch'};}
    ...
    const next=new RumbleClient(url,receive,{game:'wen-lambo',step:stepRace,phase:'racing'});client.current=next;if(kind==='resume')next.session=resume;next.connect(action);audio.current?.unlock();
  };
  ...
  const touchControl=bit=>({onPointerDown:e=>{e.preventDefault();e.currentTarget.setPointerCapture(e.pointerId);touch.current|=bit;applyInput();},onPointerUp:()=>{touch.current&=~bit;applyInput();},onPointerCancel:()=>{touch.current&=~bit;applyInput();},onLostPointerCapture:()=>{touch.current&=~bit;applyInput();}});
  ...
  return <div ref={shell} className={`${s.shell} ${playing?s.inRace:''}`}>
    <header className={s.header}><Link href='/os'>▣ TERMINL <span>ARCADE</span></Link><span className={s.preview}>WEN LAMBO / CALIFORNIA v3</span><div>{mode!=='menu'&&<button onClick={leave}>← LEAVE</button>}<button onClick={()=>{neutral();setSettings(true);if(mode==='practice')setPaused(true);}}>SETTINGS</button><button aria-label='Toggle fullscreen' onClick={()=>{if(document.fullscreenElement)document.exitFullscreen?.().catch(()=>{});else shell.current?.requestFullscreen?.().catch(()=>setNotice('Fullscreen is unavailable.'));}}>⛶</button></div></header>
    ...
    {mode==='menu'?<main className={s.menu}>
      ...
      <div className={s.hud}><div><small>POSITION</small><strong>{me?.place||'—'}<i>/2</i></strong></div><div><small>LAP</small><strong>{Math.min((me?.lap||0)+1,3)}<i>/3</i></strong></div><div><small>RACE TIME</small><b>{((view?.raceTicks||0)/60).toFixed(1)}s</b></div><div className={s.boost}><small>BOOST / HOLD SHIFT</small><div><i style={{width:`${me?.boost||0}%`}}/></div><span>{me?.drifting?`DRIFT BANK +${Math.floor(me.driftCharge)}`:`${Math.floor(me?.boost||0)}% · RELEASE DRIFT TO BANK`}</span></div><div><small>SPEED</small><strong>{me?.gear===-1?'R ':''}{Math.round((me?.speed||0)*42)}<i>km/h*</i></strong></div></div>
      <div className={s.arena} ref={arena}><canvas ref={canvas} tabIndex={0} aria-label='WEN LAMBO race. Up accelerates, left and right steer, down brakes and reverses, space drifts, Shift boosts, R recovers.'/>
        {view?.phase==='countdown'&&<div className={s.countdown}><span>{TRACKS[view.track].name}</span><strong>{Math.max(1,3-Math.floor(view.phaseTick/60))}</strong><p>HOLD ↑ TO LAUNCH</p></div>}
        {tutorial&&!paused&&view?.phase==='racing'&&<div className={s.lesson}><b>{lesson<3?`DRIVING SCHOOL ${lesson+1}/3`:'LICENSE QUESTIONABLY ACQUIRED ✓'}</b><p>{['Hold ↑ or W to accelerate. ← → steer.','Follow the road through the next checkpoint. ↓ brakes for tight turns; keep holding it at a stop to reverse.','Hold SPACE while steering through a turn. Release it to bank drift boost.','Now hold SHIFT on a straight to spend boost. Keep racing or try the two-track cup.'][lesson]}</p>{lesson===3&&<button onClick={()=>setTutorial(false)}>KEEP RACING →</button>}</div>}
        {me?.wrongWay&&view?.phase==='racing'&&<div className={s.warning}>WRONG WAY · Turn around or press R to recover</div>}
        {me?.offRoad&&!me?.wrongWay&&view?.phase==='racing'&&<div className={s.warning}>OFF ROAD · Find the tarmac</div>}
      ...
      <div className={s.controls}><p>↑ GAS · ← → STEER · ↓ BRAKE / REVERSE · SPACE DRIFT · SHIFT BOOST · R RECOVER · ESC PAUSE PRACTICE <small>*Arcade speed display</small></p>{seat?.slot!==-1&&<div className={s.touch}><div>{[[1,'LEFT'],[2,'RIGHT']].map(([bit,label])=><button key={bit} {...touchControl(bit)} aria-label={label}>{label==='LEFT'?'←':'→'}</button>)}</div><div>{[[8,'BRAKE / REV'],[16,'DRIFT'],[32,'BOOST'],[4,'GAS'],[64,'RESET']].map(([bit,label])=><button key={bit} {...touchControl(bit)} aria-label={label}>{label}</button>)}</div></div>}</div>
    </main>}
...
function VehiclePreview({vehicle,large=false}){
  const ref=useRef(null);useEffect(()=>{let disposed=false;const paint=()=>{if(disposed||!ref.current)return;const c=ref.current.getContext('2d');c.clearRect(0,0,500,250);drawRearCar(c,vehicle,{x:250,y:240,width:large?450:420,steer:1,tick:0});};paint();preloadRaceArt().then(paint);return ()=>{disposed=true;};},[vehicle,large]);
  return <canvas ref={ref} width={500} height={250} aria-label={`${VEHICLES[vehicle]?.name} arcade roadster with Pepe`}/>;
}
