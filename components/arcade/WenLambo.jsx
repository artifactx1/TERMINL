  ...
  return <div ref={shell} className={`${s.shell} ${playing?s.inRace:''}`}>
    ...
    {mode==='menu'?<main className={s.menu}>
      ...
      <div className={s.arena} ref={arena}><canvas ref={canvas} tabIndex={0} aria-label='WEN LAMBO race. Up accelerates, left and right steer, down brakes and reverses, space drifts, Shift boosts, R recovers.'/>
        ...
        {tutorial&&!paused&&view?.phase==='racing'&&<div className={s.lesson}><b>{lesson<3?`DRIVING SCHOOL ${lesson+1}/3`:'LICENSE QUESTIONABLY ACQUIRED ✓'}</b><p>{['Slide the thumb pad to steer; auto-gas launches on touch. Keyboard: ↑ or W for gas, ← → steer.','Follow the road through the next checkpoint. Hold BRAKE / REV (↓) for tight turns; keep holding it at a stop to reverse.','Hold DRIFT (SPACE) while steering through a turn. Release it to bank drift boost.','Now hold BOOST (SHIFT) on a straight to spend boost. Keep racing or try the two-track cup.'][lesson]}</p>{lesson===3&&<button onClick={()=>setTutorial(false)}>KEEP RACING →</button>}</div>}
