...
export default function MoonCampaign(){
  ...
  return <main className={s.menu}>
    ...
    <section className={s.hero}><div><span className={s.eyebrow}>ONE SMALL STEP. TEN VERY BIG PROBLEMS.</span><h1>MOON<br/><em>MISSION</em></h1><p>Take your terminal from the city to the moon. Climb rooftops, cross melting bridges, and beat the Warden waiting at the end.</p>
...
function Mission({run,onLeave,onRetry,onNext,onCheckpoint,onFinish,storageError}){
  ...
  return <section className={s.game} aria-label='Moon Mission campaign'>
    ...
    <div className={s.scene} ref={scene}><canvas ref={canvas} data-level={world.level} data-phase={phase} data-tick={view.tick} data-x={view.x} data-input={view.input} aria-label='Run with arrows. Space to jump and double jump. Stomp enemies and reach the rocket.'/>
      ...
      {phase==='ready'&&<div className={s.overlay}><div><span className={s.eyebrow}>LEVEL {world.level+1} OF 10</span><h2>{world.name}</h2><p>{world.mechanic}</p><p>Arrows / A D to run · Space to jump<br/>Release and jump again for a double jump.</p><button className={s.primary} onClick={()=>setPhase('running')}>{run.checkpoint>=0?'RESUME CHECKPOINT':'START LEVEL'} →</button><small>{Math.floor(world.timeLimit/3600)} MINUTES · 3 HEALTH · CHECKPOINTS</small></div></div>}
