...
export default function AssetLab(){
  ...
  useEffect(()=>{
    ...
    for(const id of ["dead-mall","laundromat"]){const image=new window.Image();image.onload=()=>{if(disposed)return;images.current[id]=image;if(Object.keys(images.current).length===2)setLoad("2 / 2 stage plates loaded");};image.onerror=()=>{if(!disposed)setLoad("Asset failure: "+id);};image.src=`/arcade/${id}-v1.webp`;}
    const frame=now=>{
