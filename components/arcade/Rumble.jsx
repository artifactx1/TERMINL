...
export default function Rumble({assetLabEnabled=false}){
  ...
  const practice=(teach=false)=>{
    ...
    game.current=createFight({characters:[character,opponent],stage});lessonStart.current=game.current.players[0].x;
  ...
  return <div className={`${s.shell} ${mode!=="menu"?s.inGame:""}`}>
    ...
    {mode==="menu"?<main className={s.menu}>
      ...
      <footer className={s.menuFooter}><p>Original collection-inspired game adaptations. Artwork and combat balance are in development review.</p>{assetLabEnabled&&<Link href="/os/asset-lab">OPEN ASSET / ANIMATION LAB ↗</Link>}<Link href="/os">BACK TO THE ARCADE ↗</Link></footer>
