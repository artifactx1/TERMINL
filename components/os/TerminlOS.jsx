...
export default function TerminlOS({ machines }) {
  ...
  const start = (mode, bot = "max") => {
    if (!ready) return;
    if (runRef.current && !runRef.current.finished) { setApp("arcade"); setNotice("Your run is still open. Bank your bag or use End run to start another."); return; }
    ...
    updateRun({ id: crypto.randomUUID(), seed, mode, finished: false, coin: mode === "challenge" && incoming ? incoming.coin : "COPE",
      ...
      rivalRecord: mode === "practice" ? botReplay(seed, bot) : mode === "challenge" && incoming ? {seed: incoming.seed, moves: incoming.moves, endTick: incoming.endTick} : null,
      bot: mode === "practice" ? bot : null });
    if (mode === "challenge") setIncoming(null);
    setApp("arcade"); setModal(null); beep(640);
  };
  ...
  const share = async () => {
    ...
    const code = encodeChallenge({ v: 2, ...current.record, coin: current.coin, name: profileRef.current.name.trim() || "ANON" });
  ...
  return <div className={s.os} style={{ "--accent": theme.color }}>
    ...
    <div className={s.desktop}>
      ...
      <main className={s.main}>
        ...
        {incoming && !run && app === "arcade" && <div className={s.invite}><div><b>{incoming.name} left you a challenge.</b><span>Same course. Same crowd. Beat their ghost.</span></div><button className={s.darkButton} onClick={() => start("challenge")}>Accept challenge <Icon name="swords" size={17} /></button></div>}

        <div className={s.pageHeading}><div><h1>{app === "arcade" ? "Welcome to the trenches." : app === "trophies" ? "Proof you were here." : app === "shop" ? "Spend your bad decisions." : "Make yourself at home."}</h1><p>{app === "arcade" ? "Dodge the red. Grab the green. Know when to cash out." : app === "trophies" ? "Small victories. Spectacular failures. All worth keeping." : app === "shop" ? "Entirely unnecessary upgrades. Extremely necessary energy." : "A little personality for your corner of the internet."}</p></div><div className={s.creditPill}><Icon name="bolt" size={18} /><b>{fmt(profile.credits)}</b><span>CR</span></div></div>

        {<div className={s.contentGrid} style={{ display: app === "arcade" ? undefined : "none" }}>
          <div className={s.primaryColumn}>
            {run ? run.finished ? <GameResult run={run} profile={profile} share={share} download={downloadReceipt} leave={() => updateRun(null)} /> : <RugRunner key={run.id} run={run} active={app === "arcade" && !modal} sound={profile.sound} onComplete={finishRun} leave={() => setModal("quit")} /> : <>
              <section className={s.hero}>
                <div className={s.heroNoise} /><div className={s.heroCopy}><div className={s.eyebrow}><span className={s.statusDot} /> NOW RUNNING / RUG_RUNNER.EXE</div>
                  <h2>Build a bag.<br />Dodge the rug.<br /><em>Make it out.</em></h2><p>A 45-second sprint through a meme coin meltdown.<br />Bank your score before the market eats it.</p>
                  <button className={s.accentButton} disabled={!ready} onClick={() => start("daily")}>{dailyClaimed ? "REPLAY THE DAILY" : "PLAY THE DAILY"} <Icon name="arrow" size={19} /></button>
                  <span className={s.heroFine}>45 SECONDS &nbsp;·&nbsp; FREE TO PLAY &nbsp;·&nbsp; REAL BAD IDEAS</span>
                </div><div className={s.heroArt}><div className={s.orbit} /><Image src={`/art/${machine.slug}.webp`} alt={`${machine.chassis} pixel art terminal`} width={420} height={420} priority /><span className={s.artTag}>RUG RUNNER / KEEP YOUR BAG ALIVE ↗</span></div>
              </section>
              <div className={s.ticker}><span>GREEN = BAGS</span><b>GOLD = BIG BAGS</b><b>BLUE = SHIELD</b><em>RED = REKT</em><span>3 LIVES / NO SECOND CHANCES</span></div>
              <div className={s.sectionLabel}><span>CHOOSE YOUR BAD IDEA</span><small>NO ENTRY FEES. JUST EGO.</small></div>
              <div className={s.modeGrid}>
                <button className={s.modeCard} disabled={!ready} onClick={() => start("daily")}><div className={s.modeTop}><span className={s.modeIcon}><Icon name="bolt" size={25} /></span><span className={s.tag}>{dailyClaimed ? "BONUS CLAIMED ✓" : "+100 CR DAILY"}</span></div><h3>The daily rug run</h3><p>One course. One meme coin market. Grab a bag and bank 1,000 points to win.</p><div className={s.modeBottom}><span>RESETS IN {countdown}</span><Icon name="arrow" /></div></button>
        ...
        {app !== "arcade" && run && !run.finished && <button className={s.resumeButton} onClick={() => setApp("arcade")}>↳ Resume Rug Runner</button>}
    ...
    {modal === "rivals" && <Modal title="FIND_AN_OPPONENT.EXE" close={() => setModal(null)}><span className={s.eyebrow}>PRACTICE DUELS · BOT OPPONENTS</span><h2>Choose your emotional support rival.</h2><p>Same falling candles. Same market crowd. Race their recorded ghost and bank the bigger score.</p><div className={s.rivalList}>{BOTS.map(bot => <button key={bot.id} onClick={() => start("practice", bot.id)}><Image src={`/degens/${bot.portrait}.webp`} alt="" width={68} height={112} /><div><span className={s.tag}>{bot.difficulty}</span><h3>{bot.name}</h3><p>“{bot.quote}”</p></div><Icon name="arrow" /></button>)}</div></Modal>}
    {modal === "help" && <Modal title="READ_ME.TXT" close={() => setModal(null)}><span className={s.eyebrow}>RUG RUNNER / THE ENTIRE MANUAL</span><h2>Grab green. Dodge red. Bank your bag.</h2><ol className={s.instructions}><li><b>Move between three lanes.</b> Use ← → or A / D. On mobile, tap the lane you want or use the left/right buttons.</li><li><b>Collect green candles.</b> They add free token allocations to your bag. Consecutive pickups build a multiplier up to ×5. Gold candles award three times more.</li><li><b>Dodge red candles.</b> A hit costs one life and 30% of your bag. Three hits wipe the bag out. Blue shields absorb a hit.</li><li><b>Watch the market.</b> Simulated buyers pump the token; sellers dump it. A whale warning gives you a chance to leave before a dev dump. Your exit value changes live.</li><li><b>Cash out or keep running.</b> The cash-out button unlocks after 5 seconds and banks your score immediately. Survive 45 seconds to bank automatically. Aim for 1,000 points solo, or beat your rival.</li></ol><div className={s.helpBox}><b>Yes, the curve actually changes your exit.</b><p>Pickups and simulated crowd trades move a simplified constant-product curve. Your displayed score is what selling your full bag would return, after price impact and a fictional 1% selling fee. This is an arcade model, not a copy of a live token protocol.</p></div><div className={s.helpBox}><b>Friends, ghosts, and spoils</b><p>Share your finished run as a link. A friend races your recorded ghost with the same course and crowd flow. This is asynchronous and unranked. Every finish earns 60 CR, a win adds 80 CR, and each new trophy adds 50 CR. The first daily finish each UTC day adds 100 CR.</p></div><p className={s.footnote}>Free to play. Fictional tokens and points. Credits unlock browser-local cosmetics; they cannot be redeemed for money or NFTs. No live multiplayer or global rankings. Guest stations do not prove NFT ownership.</p><button className={s.darkButton} onClick={() => setModal(null)}>GOT IT. LET ME RUN →</button></Modal>}
    {modal === "quit" && <Modal title="END_RUN.EXE" close={() => setModal(null)}><h2>Pull the plug?</h2><p>This unfinished run will be lost. Cash out during play to keep your score, or finish the run to collect rewards.</p><div className={s.buttonRow}><button className={s.darkButton} onClick={() => setModal(null)}>Keep playing</button><button className={s.outlineButton} onClick={() => { updateRun(null); setModal(null); }}>End run</button></div></Modal>}
    {modal === "share" && <Modal title="FRIENDLY_FIRE.LINK" close={() => setModal(null)}><span className={s.eyebrow}>YOUR SCORE IS THEIR PROBLEM NOW</span><h2>Bet they can&apos;t do better.</h2><p>You finished with <b>{fmt(run.summary.score)}</b>. Send this link to a friend. They race your ghost through the same candles and market conditions.</p><label className={s.shareField}>YOUR CHALLENGE LINK<textarea readOnly value={shareUrl} onFocus={e => e.target.select()} rows={3} /></label><button className={s.darkButton} onClick={copy}>COPY CHALLENGE LINK <Icon name="share" size={17} /></button><p className={s.footnote}>Asynchronous, unranked PvP. The link includes your callsign and recorded lane changes. Your friend can send their result link back; results do not sync automatically.</p></Modal>}
  </div>;
}

function Stat({ label, value }) { return <div><span>{label}</span><b>{value}</b></div>; }

function GameResult({ run, profile, share, download, leave }) {
  const result = run.summary;
  return <section className={s.gameWindow}><div className={s.windowBar}><span>RUG_RUNNER.EXE / RUN COMPLETE</span><button onClick={leave} aria-label="Close results">×</button></div><div className={s.resultTop}><span className={s.tag}>{result.dead ? "RUGGED IN THE TRENCHES" : result.tie ? "DRAW" : result.win ? "BAG SUCCESSFULLY SECURED" : "MADE IT OUT"}</span><div className={s.resultSymbol}>{result.dead ? "×" : "↗"}</div><h2>{result.title}</h2><p>${run.coin} · {profile.name} · {result.duration.toFixed(1)}s in the trenches</p><div className={s.resultScores}><div><span>POINTS BANKED</span><b>{fmt(result.score)}</b><small>{result.banked ? "CASHED OUT EARLY" : result.dead ? "THREE HITS. ZERO BAGS." : "SURVIVED THE FULL RUN"}</small></div>{result.rival !== null && <div><span>{run.rivalName}</span><b>{fmt(result.rival)}</b><small>{result.tie ? "IT'S A DRAW" : result.win ? "YOU BEAT THEIR GHOST" : "RIVAL WINS THIS ONE"}</small></div>}</div><div className={s.runnerResultStats}><span>{result.coins} CANDLES COLLECTED</span><span>{result.bestCombo} BEST STREAK</span></div></div><div className={s.resultRewards}><div><Icon name="bolt" size={28} /><div><b>+{result.reward} CR</b><span>60 finish{result.win ? " + 80 win" : ""}{result.dailyBonus ? " + 100 daily" : ""}{result.unlocked.length ? ` + ${result.unlocked.length * 50} trophies` : ""}</span></div></div><span>+{result.win ? 150 : 100} XP</span></div>{result.unlocked.length > 0 && <div className={s.unlocks}>{result.unlocked.map(id => { const t = TROPHIES.find(t => t.id === id); return <span key={id}>{t.symbol} {t.name} <small>UNLOCKED</small></span>; })}</div>}<div className={s.resultActions}><button className={s.accentButton} onClick={share}>CHALLENGE A FRIEND <Icon name="swords" size={18} /></button><button className={s.outlineButton} onClick={download}>↓ SAVE RECEIPT</button><button className={s.textButton} onClick={leave}>Back to arcade <Icon name="arrow" size={16} /></button></div></section>;
}
