...
import { newMoon, moonBotReplay } from "../../lib/moon-mission.mjs";
...
export default function TerminlOS({ machines }) {
  ...
  const start = (mode, bot = "max", gameId = selectedGame) => {
    if (!ready) return;
    ...
    updateRun({ id: crypto.randomUUID(), seed, mode, game, finished: false, coin: mode === "challenge" && incoming ? incoming.coin : game === "moon" ? "MOON" : "COPE",
      ...
      rivalRecord: mode === "practice" ? (game === "moon" ? moonBotReplay : botReplay)(seed, bot) : mode === "challenge" && incoming ? {seed: incoming.seed, moves: incoming.moves, endTick: incoming.endTick} : null,
  ...
  return <div className={s.os} style={{ "--accent": theme.color }}>
    ...
    <div className={s.desktop}>
      ...
      <main className={s.main}>
        ...
        <div className={s.invite}><div><b>NEW / WEN LAMBO</b><span>Finally rich. Still can’t drive. Two pixel cars, two tracks, drift-to-boost racing and a private two-player cup.</span></div><Link className={s.darkButton} href="/os/lambo">PLAY WEN LAMBO ↗</Link></div>
        ...
        <div className={s.pageHeading}><div><h1>{app === "arcade" ? "Welcome to the trenches." : app === "trophies" ? "Proof you were here." : app === "shop" ? "Spend your bad decisions." : "Make yourself at home."}</h1><p>{app === "arcade" ? "Four games. One terminal. Absolutely no adult supervision." : app === "trophies" ? "Small victories. Spectacular failures. All worth keeping." : app === "shop" ? "Entirely unnecessary upgrades. Extremely necessary energy." : "A little personality for your corner of the internet."}</p></div><div className={s.creditPill}><Icon name="bolt" size={18} /><b>{fmt(profile.credits)}</b><span>CR</span></div></div>

        {<div className={s.contentGrid} style={{ display: app === "arcade" ? undefined : "none" }}>
          <div className={s.primaryColumn}>
            {run ? run.finished ? <GameResult run={run} profile={profile} share={share} download={downloadReceipt} leave={() => updateRun(null)} /> : run.game === "moon" ? <MoonMission key={run.id} run={run} machine={machine} active={app === "arcade" && !modal} sound={profile.sound} onComplete={finishRun} leave={() => setModal("quit")} /> : <RugRunner key={run.id} run={run} active={app === "arcade" && !modal} sound={profile.sound} onComplete={finishRun} leave={() => setModal("quit")} /> : <>
              ...
              <section className={s.hero}>
                <div className={s.heroNoise} /><div className={s.heroCopy}><div className={s.eyebrow}><span className={s.statusDot} /> NOW RUNNING / {selectedGame === "moon" ? "MOON_MISSION.EXE" : "RUG_RUNNER.EXE"}</div>
                  {selectedGame === "moon" ? <><h2>Little CRT.<br />Big adventure.<br /><em>To the moon.</em></h2><p>Run, double-jump, and stomp through a neon world.<br />Find the coins. Reach the rocket.</p></> : <><h2>Build a bag.<br />Dodge the rug.<br /><em>Make it out.</em></h2><p>A 45-second sprint through a meme coin meltdown.<br />Bank your score before the market eats it.</p></>}
                  ...
                  <span className={s.heroFine}>{selectedGame === "moon" ? "A FULL PLATFORM ADVENTURE" : "45 SECONDS"} &nbsp;·&nbsp; FREE TO PLAY</span>
                </div>{selectedGame === "moon" ? <div className={s.moonFeatureArt}><MoonCover /></div> : <div className={s.heroArt}><div className={s.orbit} /><Image src={`/art/${machine.slug}.webp`} alt={`${machine.chassis} pixel art terminal`} width={420} height={420} priority /><span className={s.artTag}>RUG RUNNER / KEEP YOUR BAG ALIVE ↗</span></div>}
              </section>
