 ...
 *
 * Browser reads try the restricted site API first and use the public endpoint
 * as transport fallback. On the server, `RPC_URL` is preferred when set, with the
 ...
 * CDN cache keeps shared drop reads low; unique-wallet traffic still consumes
 * provider capacity. The fallback means a
...
async function post(url, body) {
  ...
  try {
    res = await fetch(url, {
      ...
      signal: AbortSignal.timeout(url.startsWith("/") ? 15_000 : RPC_TIMEOUT_MS),
    });
    if (!res.ok) throw new RpcTransportError(`RPC unavailable (${res.status}). Please retry.`);
    return await res.json();
  } catch (e) {
    // Never expose a provider URL (which can contain a private API key).
    throw new RpcTransportError(e instanceof RpcTransportError ? e.message : "Chain connection timed out or is unavailable. Please retry.");
  }
}

function unwrap(json) {
  if (!json || typeof json !== "object") throw new RpcTransportError("Invalid RPC response.");
  if (json.error) {
    // Providers also report throttling inside HTTP 200 JSON-RPC responses.
    // Preserve actual reverts and their data instead of retrying them.
    if ([-32005, -32016].includes(json.error.code)
      || (json.error.code !== 3 && !json.error.data
        && /rate.?limit|too many requests|compute units|capacity|temporarily unavailable|timeout|timed out/i.test(json.error.message || ""))
    ) throw new RpcTransportError("RPC is busy. Please retry.");
  ...
  if (!("result" in json)) throw new RpcTransportError("Missing RPC result.");
  return json.result;
}
...
async function withFallback(fn) {
  ...
  for (let i = 0; i < urls.length; i += 1) {
    if (urls.length > 1 && i < urls.length - 1 && (unhealthyUntil.get(urls[i]) || 0) > Date.now()) continue;
    try {
      const out = await fn(urls[i]);
      lastRpcSource = urls[i] !== RPC_URL ? "keyed" : "public";
      return out;
    } catch (e) {
      if (!(e instanceof RpcTransportError)) throw e;
      unhealthyUntil.set(urls[i], Date.now() + 15_000);
      lastError = e;
    }
  }
  throw lastError;
...
async function rpc(method, params) {
  if (typeof window !== "undefined") {
    try {
      return unwrap(await post("/api/mint-rpc", { jsonrpc: "2.0", id: 1, method, params }));
    } catch (e) {
      if (!(e instanceof RpcTransportError)) throw e;
      // A site outage must not prevent confirmation of an already-sent mint.
    }
  }
  return withFallback(async (url) => unwrap(await post(url, { jsonrpc: "2.0", id: 1, method, params })));
}

// Server route validates the method and fixed-contract arguments before calling.
export const requestMintRpc = (method, params) => rpc(method, params);
...
async function rpcBatch(calls) {
  return withFallback(async (url) => {
    ...
    return calls.map((_, i) => {
      if (!byId.has(i + 1)) throw new RpcTransportError("Missing batch reply.");
      return unwrap(byId.get(i + 1));
    });
  });
}
...
export async function readDrop() {
  ...
  return {
    ...describeDrop(facts.condition, facts.endsAt, facts.chainNow, facts),
 ...
 */
export function describeDrop({ startsAt, maxClaimable, claimed, perWallet, price, currency }, endsAt, now, totals = {}) {
  ...
  /*
   * The drop's own totals, which are NOT the phase's.
   *
   * `claimed` counts claims through the PUBLIC condition alone — a stage mint
   * goes through claimAllowlist and never touches it — and `maxClaimable` is
   * whatever slice of the collection the artist gave this phase. So a drop of
   * 10000 with a public phase capped at 6998 was reporting "6989 left" while
   * 9990 tokens actually remained, and a progress bar reading 7 / 6998 for a
   * collection of 10000.
   *
   * `minted` (the contract's monotonic counter) and `lazySupply` (how many
   * exist at all) are the drop's real figures, and the claim is bounded by
   * them too: verifyClaim reverts once a claim would pass the lazy-minted
   * supply, whatever the condition allows.
   */
  const dropMinted = totals.minted ?? claimed;
  const dropSupply = totals.lazySupply || maxClaimable;
  const lazyLeft = totals.lazySupply ? (totals.lazySupply > totals.minted ? totals.lazySupply - totals.minted : 0n) : null;

  const phaseLeft = maxClaimable > claimed ? maxClaimable - claimed : 0n;
  const trulyLeft = lazyLeft === null ? phaseLeft : (phaseLeft < lazyLeft ? phaseLeft : lazyLeft);

  const soldOut = configured && trulyLeft === 0n;
  ...
  return {
    ...
    nobodyMayMint,
    /* What this PHASE has taken and may still take. */
    minted: claimed,
    supply: maxClaimable,
    remaining: trulyLeft,
    /* What the DROP has taken and holds — for the progress bar, which is
     * about the collection rather than about whichever phase is open. */
    dropMinted,
    dropSupply,
    soldOut,
  };
...
export async function fetchDropFacts() {
  if (!CONTRACT) return null;
  try {
    const res = await fetch("/api/drop", { cache: "no-store", signal: AbortSignal.timeout(15_000) });
