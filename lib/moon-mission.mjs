...
import { createMoonWorlds } from "./moon-worlds.mjs";
...
export const MOON_WORLDS = createMoonWorlds({ground:GROUND,platforms:PLATFORMS,pickups:PICKUPS,enemies:ENEMIES,checkpoints:CHECKPOINTS});
export function moonWorld(level=0) {
  if(!Number.isInteger(level)||!MOON_WORLDS[level])throw Error("Unknown Moon Mission world");
  return MOON_WORLDS[level];
}
export function newMoon(seed,level=0) {
  moonWorld(level);
  return {seed,level,tick:0,input:0,previousInput:0,x:65,y:426,vx:0,vy:0,face:1,grounded:true,jumps:0,coyote:8,
...
export function stepMoon(prev) {
  ...
  const world=moonWorld(prev.level);
  ...
  s.x=Math.max(0,Math.min(world.width-PLAYER_W,s.x+s.vx*DT));
  ...
  for(const raw of world.enemies){
  ...
  for(const x of world.checkpoints)if(s.x>=x&&s.checkpoint<x){s.checkpoint=x;s.pulse={tick:s.tick,text:"CHECKPOINT SAVED",kind:"checkpoint"};}
  s.furthest=Math.max(s.furthest,s.x);
  if(s.y>620)s=damage(s,true);
  if(s.x>=world.finish&&!s.dead){s.won=true;s.score+=1000+Math.max(0,Math.floor((MOON_END_TICK-s.tick)/6));}
...
export function validateMoonReplay(data) {
  ...
  if(data.level!==undefined&&(!Number.isInteger(data.level)||!MOON_WORLDS[data.level]))return null;
  let last=-1;
  ...
  return {seed:data.seed,...(data.level===undefined?{}:{level:data.level}),endTick:data.endTick,moves:data.moves.map(({tick,input})=>({tick,input}))};
}
export function replayMoon(record) {
  const valid=validateMoonReplay(record);if(!valid)throw Error("Invalid Moon Mission replay");
  let s=newMoon(valid.seed,valid.level),index=0;
...
export function moonBotReplay(seed,id="chloe",level=0) {
  const world=moonWorld(level);
  let s=newMoon(seed,level),moves=[],holdUntil=0;
  while(!s.ended){
    let input=RIGHT;
    const ahead=s.x+105;
    const gap=!world.ground.some(([a,b])=>ahead>=a&&ahead<b);
    const enemy=world.enemies.some(e=>!s.defeated.includes(e.id)&&enemyAt(e,s.tick,seed).x>s.x&&enemyAt(e,s.tick,seed).x-s.x<85&&s.y>380);
    if(s.grounded&&(gap||enemy)&&s.tick>holdUntil+2)holdUntil=s.tick+18;
    // New-world ledges can end above a gap. Recover with the available air jump
    // instead of waiting forever for a ground jump. Preserve legacy bot inputs.
    if(level>0&&!s.grounded&&s.jumps===0&&gap&&s.vy>0&&s.tick>holdUntil+2)holdUntil=s.tick+18;
  ...
  return {seed,...(level?{level}:{}),moves,endTick:s.tick};
}
