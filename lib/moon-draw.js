...
export function drawMoon(ctx,s,width,{art=null,ghost=null,world:customWorld=null,enemyPosition=enemyAt}={}) {
  const world=customWorld||moonWorld(s.level),{platforms:PLATFORMS,pickups:PICKUPS,enemies:ENEMIES,checkpoints:CHECKPOINTS,ground:GROUND}=world;
  ...
  for(const raw of ENEMIES){if(s.defeated.includes(raw.id))continue;const e=enemyPosition(raw,s.tick,s.seed);if(e.x-camera< -50||e.x-camera>width+50)continue;const step=Math.sin(s.tick*.2)*3;
  ...
  for(const point of CHECKPOINTS){const x=typeof point==='number'?point:point.x,y=typeof point==='number'?470:point.y+44,on=s.checkpoint>=x;ctx.fillStyle="#859890";ctx.fillRect(x,y-83,4,83);ctx.fillStyle=on?"#9bfd7f":"#638b8e";ctx.fillRect(x+4,y-80,32,19);ctx.fillStyle="#1a3824";ctx.font="10px monospace";ctx.textAlign="left";ctx.fillText(on?"✓":"⚑",x+14,y-66);}
