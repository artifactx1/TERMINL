...
export async function saveReceipt(run, profile, machine) {
  ...
  ctx.font = "16px monospace"; ctx.fillText(moon ? "MOON MISSION / FLIGHT REPORT" : "RUG RUNNER / OFFICIAL DAMAGE REPORT", 95, 150);
  ...
  const stats = [
    ...
    `${moon ? "COINS COLLECTED        " : "CANDLES COLLECTED      "} ${run.summary.coins}`,
    `${moon ? "ENEMIES STOMPED        " : "BEST PICKUP STREAK     "} ${moon ? run.summary.stomps : run.summary.bestCombo}`,
    `EXIT STATUS            ${moon ? run.summary.won ? "ROCKET REACHED" : run.summary.dead ? "OUT OF LIVES" : "TIME RAN OUT" : run.summary.dead ? "COMPLETELY RUGGED" : run.summary.banked ? "CASHED OUT EARLY" : "SURVIVED"}`,
