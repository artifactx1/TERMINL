...
function carSprite(id,steer){
  ...
  const s=surface(66,42);if(!s)return null;const c=s.getContext('2d');c.translate(33,21);const exotic=id==='spectre',paint=exotic?'#e38e61':'#a2ce62',light=exotic?'#ffd3a0':'#e5ffc1',dark=exotic?'#8f493c':'#4f7846';
...
function cachedTrack(id){
  ...
  const entry={canvas,geometry:g};tracks.set(id,entry);return entry;
}
...
function minimap(c,state,slot,width,height){
  const w=Math.min(166,width*.22),h=w*.72,x=width-w-15,y=height-h-15,g=trackGeometry(state.track),scale=(w-18)/WORLD.width;
  c.save();rect(c,x,y,w,h,'#061018dc');rect(c,x,y,w,2,'#6b8874');c.translate(x+9,y+9);c.scale(scale,scale);stroke(c,g.points,'#63736e',44,true);
  const next=g.gates[state.players[slot]?.nextCheckpoint||0];rect(c,next.x-30,next.y-30,60,60,'#e9e4af');
  for(let i=state.players.length-1;i>=0;i--){const p=state.players[i];rect(c,p.x-40,p.y-40,80,80,'#081218');rect(c,p.x-25,p.y-25,50,50,COLORS[i]);}c.restore();
  text(c,'CIRCUIT / NEXT GATE',x+8,y+h-5,8,'#afbeac','left');
}

export function drawRace(ctx,state,{width=1000,height=600,slot=0,reducedMotion=false,quality='high',overview=false,debug=false}={}){
  if(!overview){drawPerspectiveRace(ctx,state,{width,height,slot,reducedMotion,quality});minimap(ctx,state,slot,width,height);return;}
...
export const RACE_ART_MANIFEST=Object.freeze({version:1,vehicles:['comet','spectre'],tracks:['night-market','liquidation-docks'],source:'original-code-native',nativeCarPixels:[66,42],trackPixels:[1168,864],maxTrackCacheBytes:8073216,liveEffects:['steering','boost-exhaust','drift-smoke','tire-marks','ghost-recovery','checkpoint','minimap']});
