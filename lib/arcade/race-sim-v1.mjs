/** WEN LAMBO v1. Original 2D arcade handling; fixed 60 Hz, input-only authority. */
...
export const CUP_TRACKS=Object.freeze(['night-market','liquidation-docks']);
...
export function createRace({vehicles=['comet','spectre'],track='night-market',cup=true}={}){
  ...
  return {version:1,game:'wen-lambo',tick:0,phase:'countdown',phaseTick:0,track,tracks,trackIndex:0,players:vehicles.map((v,i)=>car(v,i,track)),raceTicks:0,firstFinish:null,winner:null,wins:[0,0],events:[],raceResults:[],bounds:null};
}
...
function recover(s,p,slot,g){
  ...
  p.x=q(x);p.y=q(y);p.angle=at.angle;p.vx=0;p.vy=0;p.speed=0;p.boost=Math.max(0,p.boost-15);p.driftCharge=0;p.resetCooldown=180;p.ghost=90;event(s,'reset',slot,'BACK TO YOUR LAST CHECKPOINT');
}
function drive(s,p,slot,mask,g){
  ...
  const speed=Math.hypot(p.vx,p.vy),signedSpeed=p.vx*Math.cos(p.angle)+p.vy*Math.sin(p.angle),direction=signedSpeed<-.1?-1:1;
  p.drifting=!!(mask&16)&&signedSpeed>2&&!p.offRoad&&!!targetSteer;
  const targetYaw=p.steer*v.steer*direction*clamp(speed/2,0,1)*(p.drifting?1.15:1)*Math.max(.5,1-speed/15);
  p.yawRate=q((p.yawRate||0)+(targetYaw-(p.yawRate||0))*.12);p.angle=q(p.angle+p.yawRate);
  let forward=p.vx*Math.cos(p.angle)+p.vy*Math.sin(p.angle);
  if(mask&4)forward+=forward<-.1?.15:v.acceleration;
  if(mask&8)forward-=forward>.1?.15:.06;
  ...
  if(p.offRoad)forward=clamp(forward*.95,-1.5,2.0);
  const grip=p.drifting?v.driftGrip:v.grip;
  ...
  const after=nearestRoad(g,p.x,p.y);p.wrongWay=p.speed>1&&Math.abs(angleDelta(p.angle,after.angle))>Math.PI*.65;
  if(after.distance>g.width/2+55){const nx=(p.x-after.x)/after.distance,ny=(p.y-after.y)/after.distance;p.x=q(after.x+nx*(g.width/2+55));p.y=q(after.y+ny*(g.width/2+55));p.vx=q(p.vx*.2);p.vy=q(p.vy*.2);p.speed=q(p.speed*.2);p.driftCharge=0;if(s.tick%20===0)event(s,'wall',slot,'BARRIER · SPEED LOST');}
