/** WEN LAMBO v4. Fixed 60 Hz with proportional steering. Archived v1-v3 replays remain exact. */
...
export const RACE_RULES_VERSION=4;
...
function drive(s,p,slot,mask,g){
  ...
  const lock=p.drifting?1:1-.34*pace*pace;
  ...
  p.yawRate=q(oldYaw+(targetYaw-oldYaw)*(requested===0||reversing?.45:.30));p.angle=q(p.angle+p.yawRate);
