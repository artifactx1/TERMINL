...
import { drawFinisher } from './rumble-finisher-render.js';
...
function expansionProp(c,at,id,tick,active,face){
  ...
  if(id==='brian'){
    ...
    if(active){ellipse(c,5,0,24,13,'#ff86c233','#ff86c2',2);line(c,[[-19,5],[5,-6],[29,0]],'#ffc1e5',3);}
  }else if(id==='bernie'){
...
export function drawRumble(c,state,{width=1000,height=600,images={},debug=false,reducedMotion=false,quality='high',interpolation=0,effectTick=state?.tick,shake=true,pixelStyle=true}={}){
  ...
  if(state.phase==='finisher'){drawFinisher(c,state,{images,reducedMotion,shake,drawFighter});c.restore();return;}
  ...
  // Both fighters stand on one floor line, so overlap needs a deliberate layering
  // rule rather than slot order: shadows first, then the fighter committing to an
  // attack, then an airborne one, draws in front. Ties keep the second seat in front,
  // which is what the arena always showed, so nothing flickers while both stand idle.
  const fighters=state.players.map((f,i)=>({f,i,x:f.x+(f.vx||0)*interpolation}));
  for(const {f,x} of fighters)ellipse(c,x,506,Math.max(18,42-(f.y||0)*.08),8,'#0409128c',null);
  const layer=({f,i})=>(f.action?2:0)+(f.grounded===false?1:0)+i*.5;
  for(const {f,i,x} of [...fighters].sort((a,b)=>layer(a)-layer(b))){
    ...
    const finishTarget=state.phase==='finishWindow'&&state.finisher.player!==i;
    drawFighter(c,{...f,x,...(finishTarget?{hp:1,stun:0,knockdown:0,previewClip:'hit'}:{}),landing:landing?1:0,victory:winner||f.victory},{tick,scale:1,debug,pixelStyle,image:images[f.character]});
  }
