/**
 * REKT RUMBLE development art, authored in canvas. No portrait is used as a
 * gameplay sprite. The rig is a set of independently posed body segments;
...
export function attachmentAnchors(f,tick=0){
  ...
  return {feet:[0,0],hip:p.hip,chest:p.chest,head:p.head,hat:[p.head[0],p.head[1]-34],chain:[p.chest[0]+3,p.chest[1]+(f.character==='diamond'?14:32)],frontHand:p.frontHand,rearHand:p.rearHand,effect:p.frontHand};
}
...
export function drawFighter(c,f,{tick=0,scale=1,debug=false,preview=false,pixelStyle=true}={}){
...
function effects(c,state,tick,reducedMotion){
  ...
  if(state.phase==='roundOver'||state.phase==='finished'){
    ...
    if(losing){
      ...
      if(age>25)label(c,'YOUR RECEIPT IS READY',clamp(losing.x,160,840),556,13,'#d7b3a3');
    }
  }
}
...
export function drawRumble(c,state,{width=1000,height=600,images={},debug=false,reducedMotion=false,quality='high',interpolation=0,effectTick=state?.tick,shake=true,pixelStyle=true}={}){
  ...
  state.players.forEach((f,i)=>{
    ...
    drawFighter(c,{...f,x,landing:landing?1:0,victory:winner||f.victory},{tick,scale:1,debug,pixelStyle});
  });
