 ...
 */
import {trackGeometry,nearestRoad,roadAt,angleDelta,VEHICLES,GATE_MARGIN} from './race-sim.mjs';
import {drawVehicleArt,preloadVehicleArt,VEHICLE_ART} from './race-exotics.js';
import {drawRaceWheels} from './race-wheels.js';
import {CAR_WIDTH,raceCameraLayout,raceCameraTarget,raceProjection,finishLineTiles} from './race-camera.mjs';
...
export function preloadRaceArt(){
  ...
  loading=Promise.all([preloadVehicleArt(),...Object.entries(files).map(([key,src])=>new Promise(resolve=>{const im=new Image();im.onload=()=>{cache[key]=im;resolve();};im.onerror=()=>resolve();im.src=src;}))]);return loading;
}
const clamp=(v,a,b)=>Math.max(a,Math.min(b,v));
export const playerCarFrame=(width,height)=>{const rig=raceCameraLayout(width,height);return {x:width/2,y:rig.feet,width:CAR_WIDTH*rig.focal/rig.back};};
...
export function drawRearCar(c,id,{x,y,width=260,steer=0,boosting=false,braking=false,tick=0,ghost=false,rotation=0,view=0}={}){
  preloadRaceArt();width*=VEHICLE_ART[id]?.raceWidth||1;const h=width*.72;c.save();c.globalAlpha=ghost?.72:1;
  ...
  if(!drawVehicleArt(c,id,{x,y,width,rotation})){
    if(!VEHICLE_ART[id])drawRaceWheels(c,{x,y,width,height:h,steer,rotation,view});
    if(cache.cars&&!VEHICLE_ART[id]){const cell={image:cache.cars,x:0,y:id==='spectre'?550:80,w:485,h:id==='spectre'?380:360};drawCell(c,cell,x-width/2,y-h*.84,width,h*.86);}
    else{poly(c,[{x:x-width*.43,y},{x:x-width*.49,y:y-h*.35},{x:x-width*.30,y:y-h*.85},{x:x+width*.30,y:y-h*.85},{x:x+width*.49,y:y-h*.35},{x:x+width*.43,y}],VEHICLES[id]?.color||'#bbed74');c.fillStyle='#193c50';c.fillRect(x-width*.27,y-h*.75,width*.54,h*.25);c.fillStyle='#172225';c.fillRect(x-width*.4,y-h*.2,width*.8,h*.15);}
  }
  if(braking&&id!=='bike-tyson'){c.fillStyle='#ff443777';c.fillRect(x-width*.32,y-h*.23,width*.2,h*.05);c.fillRect(x+width*.12,y-h*.23,width*.2,h*.05);}c.restore();
}
export function drawGarageVehicle(c,id,{x=250,y=240,width=460,height=225}={}){
  preloadRaceArt();
  if(!drawVehicleArt(c,id,{x,y,width,height,view:'garage'}))drawRearCar(c,id,{x,y,width:Math.min(width,350),steer:0});
}
...
export function drawPerspectiveRace(ctx,state,{width,height,slot=0,reducedMotion=false,quality='high'}={}){
  ...
  const dt=clamp((now-camera.time)/1000,0,.05),blend=1-Math.exp(-(p.digitalSteering?12:7)*dt);camera.angle+=angleDelta(p.angle,camera.angle)*blend;camera.x+=(p.x-camera.x)*(1-Math.exp(-24*dt));camera.y+=(p.y-camera.y)*(1-Math.exp(-24*dt));camera.tick=state.tick;camera.time=now;
  const heading=camera.angle,targetBack=raceCameraTarget(state,{width,height,slot,heading,roadWidth:g.width});
  camera.back=Math.max(targetBack,(camera.back??targetBack)+(targetBack-(camera.back??targetBack))*(1-Math.exp(-4*dt)));
  const {focal,horizon,camHeight,view,project,carFrame}=raceProjection(state,{width,height,slot,heading,roadWidth:g.width,back:camera.back}),far=1300;
  ...
  // Permanent timing stripe: exactly the plane and lane bounds used by drive().
  for(const tile of finishLineTiles(g))polygon(tile.points,tile.color);
  // The finish remains marked even when either car's next checkpoint advances.
  const gateLabels=[];
  for(const index of new Set([0,p.nextCheckpoint])){
  const gate=g.gates[index],left=edge(gate,-1,g.width/2+GATE_MARGIN),right=edge(gate,1,g.width/2+GATE_MARGIN);
  ...
  const gv=view(gate.x,gate.y,80);if(gv.z>50&&gv.z<900)gateLabels.push({at:project(gv),text:index===0?'FINISH / START':'CHECKPOINT',size:clamp(20*focal/gv.z,9,24)});
  }
  const objects=roadside(g).map(o=>({...o,v:view(o.x,o.y)})).filter(o=>o.v.z>15&&o.v.z<far);
  state.players.forEach((car,i)=>{const v=view(car.x,car.y);if(v.z>15&&v.z<far)objects.push({car,i,v});});
  // Decorative billboards must not hide a passing car. Both cars share depth
  // ordering, then critical gate labels stay readable above scenery.
  objects.sort((a,b)=>Number(!!a.car)-Number(!!b.car)||b.v.z-a.v.z);
  for(const o of objects){const at=project(o.v),scale=focal/o.v.z;if(o.car){
    const car=o.car,feet=carFrame(car),carWidth=feet.width;
    if(car.drifting&&!reducedMotion&&quality!=='low'){for(let i=0;i<8;i++){const age=(state.tick+i*7)%45;ctx.fillStyle=`rgba(242,233,207,${.23*(1-age/45)})`;ctx.beginPath();ctx.arc(feet.x+(i%2?1:-1)*(carWidth*.3+age)*scale*.15,feet.y+age*.3,8+age*.45,0,Math.PI*2);ctx.fill();}}
    drawRearCar(ctx,car.vehicle,{...feet,steer:car.steer,rotation:car.wheelRotation||0,view:angleDelta(car.angle,heading),boosting:car.boosting,braking:!!(car.previousInput&8),tick:state.tick,ghost:car.ghost>0});
    if(o.i!==slot)label(ctx,car.finishedTick!==null?'RIVAL · FINISHED':'RIVAL',at.x,at.y-32*scale,clamp(12*scale,9,18),'#ff80bd');
  }else if(o.biome){const w=(o.biome==='vineyard'?90:70)*scale,h=(o.biome==='vineyard'?65:o.biome==='redwood'?260:190)*scale;if(at.x+w/2>=0&&at.x-w/2<=width)drawCourseProp(ctx,o.biome,at.x,at.y,w,h);}else if(cache.scenery){const w=o.width*scale,h=o.height*scale;if(at.x+w/2<0||at.x-w/2>width)continue;const [x,y,fw,fh]=SCENERY_FRAMES[o.kind];drawCell(ctx,{image:cache.scenery,x,y,w:fw,h:fh},at.x-w/2,at.y-h,w,h);}}
  for(const gate of gateLabels)label(ctx,gate.text,gate.at.x,gate.at.y,gate.size,'#edffb2');
