...
import {drawExoticBody} from './race-exotics.js';
...
export function drawRearCar(c,id,{x,y,width=260,steer=0,boosting=false,braking=false,tick=0,ghost=false,rotation=0,view=0}={}){
  ...
  if(drawExoticBody(c,id,{x,y,width,height:h,steer,tick,braking})){/* Articulated exotic cockpit/body. */}
  else if(cache.cars){const cell={image:cache.cars,x:0,y:id==='spectre'?550:80,w:485,h:id==='spectre'?380:360};drawCell(c,cell,x-width/2,y-h*.84,width,h*.86);}
