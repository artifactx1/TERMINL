...
export class RollbackFight {
  constructor(options={}) {
    ...
    this.state=createFight(options);
  ...
  submit(slot,tick,input,seq) {
    ...
    if(!Number.isSafeInteger(tick)||tick<0||!Number.isInteger(input)||input<0||input>2047||!Number.isSafeInteger(seq)||seq<0)return {ok:false,reason:'schema'};
    ...
    return {ok:true,rolledBack,tick:this.state.tick,hash:stateHash(this.state)};
  }
  heldBefore(tick) {
  ...
  advance(inputs) {
    ...
    this.state=stepFight(this.state,this.held);
  ...
  exportReplay() {
    return {version:1,game:'rekt-rumble',options:structuredClone(this.options),ticks:this.state.tick,
      inputs:[...this.timeline].sort((a,b)=>a[0]-b[0]).map(([tick,slots])=>({tick,slots:structuredClone(slots)})),
      hash:stateHash(this.state),confirmed:this.confirmed,winner:this.confirmed?this.state.winner:null};
  }
}

export function replayFight(replay) {
  if(replay?.version!==1||replay.game!=='rekt-rumble'||!Number.isInteger(replay.ticks)||replay.ticks<0||replay.ticks>21000||!Array.isArray(replay.inputs)||replay.inputs.length>21000)throw new Error('Invalid replay');
  let state=createFight(replay.options),held=[0,0],index=0,previous=-1;
  for(const entry of replay.inputs) {
    ...
    for(const item of entry.slots)if(item!==null&&(!Number.isInteger(item.input)||item.input<0||item.input>2047))throw new Error('Invalid replay input');
    previous=entry.tick;
  }
