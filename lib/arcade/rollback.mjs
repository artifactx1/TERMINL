...
/** 300 ms of late input tolerance: a latency spike costs a correction, not a dropped control. */
export const ROLLBACK_WINDOW=18;
...
export class RollbackFight {
  constructor(options={},rules=FIGHT_RULES) {
    ...
    // Held masks used to step each retained tick, so rewinds never scan the whole timeline.
    this.heldHistory=new Map();
  ...
  submit(slot,tick,input,seq) {
    ...
    // A late packet that repeats the mask already simulated for that tick changes no
    // outcome; recording it is enough. Rewinds are reserved for actual timeline changes.
    const redundant=tick<this.state.tick&&this.heldHistory.get(tick)?.[slot]===input;
    entry[slot]={input,seq};this.timeline.set(tick,entry);this.lastSeq[slot]=seq;
    const rolledBack=tick<this.state.tick&&!redundant;
  ...
  heldBefore(tick) {
    ...
    const recorded=this.heldHistory.get(tick-1);
  ...
  rewind(tick) {
    ...
    for(const key of this.heldHistory.keys())if(key>=tick)this.heldHistory.delete(key);
  ...
  advance(inputs) {
    ...
    this.heldHistory.set(this.state.tick,[...this.held]);
    ...
    for(const tick of this.heldHistory.keys())if(tick<this.state.tick-HISTORY_LIMIT)this.heldHistory.delete(tick);
    return this.state;
  }
  exportReplay() {
    return {version:1,game:this.rules.id,rulesVersion:this.state.version,options:structuredClone(this.options),ticks:this.state.tick,
...
export function replayFight(replay,rules=FIGHT_RULES) {
  ...
  let state=rules.create({...replay.options,rulesVersion:replay.rulesVersion??1}),held=[0,0],index=0,previous=-1;
