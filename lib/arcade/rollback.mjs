...
export class RollbackFight {
  ...
  exportReplay() {
    return {version:1,game:this.rules.id,options:structuredClone(this.options),ticks:this.state.tick,
...
export function replayFight(replay,rules=FIGHT_RULES) {
  const limit=rules.id==='wen-lambo'?20000:21000;
  ...
  let state=rules.create(replay.options),held=[0,0],index=0,previous=-1;
  for(const entry of replay.inputs) {
    ...
    for(const item of entry.slots)if(item!==null&&(!Number.isInteger(item.input)||item.input<0||item.input>rules.maxInput))throw new Error('Invalid replay input');
    previous=entry.tick;
  }
