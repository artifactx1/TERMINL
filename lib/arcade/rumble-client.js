import { stepFight, RUMBLE_RULES_VERSION } from "./rumble-sim.mjs";
...
export class RumbleClient {
  constructor(url, notify, {game='rekt-rumble',step=stepFight,phase='fight',rulesVersion=RUMBLE_RULES_VERSION,graceMs=15000}={}) {
  ...
  connect(action) {
    ...
    socket.onmessage=event=>{
      ...
      if(message.type==='welcome'){
        ...
        if(this.session)this.send({type:'resume',code:this.session.code,session:this.session.session,game:this.game,rulesVersion:this.rulesVersion});
