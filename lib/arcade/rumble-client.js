...
export class RumbleClient {
  constructor(url, notify, {game='rekt-rumble',step=stepFight,phase='fight'}={}) {
    this.game=game;this.step=step;this.activePhase=phase;this.storageKey=game==='rekt-rumble'?'terminl:rumble-session':'terminl:race-session';
  ...
  connect(action) {
    ...
    socket.onmessage=event=>{
      ...
      if(message.type==='welcome'){
        ...
        if(!supported.includes(this.game)||action?.character&&!choices?.includes(action.character)){
        ...
        if(this.session)this.send({type:'resume',code:this.session.code,session:this.session.session,...(this.game==='wen-lambo'?{game:this.game}:{})});
  ...
  predictedState(){
    ...
    if(this.game==='wen-lambo'&&this.previousState?.track===this.state.track&&this.previousState.phase===this.state.phase){
      const t=Math.min(1,(performance.now()-this.snapshotAt)/50);
      predicted.players.forEach((p,i)=>{if(i===this.slot)return;const a=this.previousState.players[i],b=this.state.players[i];if(Math.hypot(a.x-b.x,a.y-b.y)<180){p.x=a.x+(b.x-a.x)*t;p.y=a.y+(b.y-a.y)*t;const angle=((b.angle-a.angle+Math.PI)%(2*Math.PI)+2*Math.PI)%(2*Math.PI)-Math.PI;p.angle=a.angle+angle*t;}});
    }
