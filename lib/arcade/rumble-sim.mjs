import { FINISHERS, FINISH_WINDOW_TICKS, FINISHER_IMPACT_TICK, FINISHER_DURATION_TICKS } from './rumble-finishers.mjs';
export const RUMBLE_RULES_VERSION = 2;
/** REKT RUMBLE rules v2 (with archived v1 replay support). Pure 60 Hz simulation; no clocks, assets or random sources. */
...
export const CHARACTERS = Object.freeze({
  max: { id:'max', name:'Margin Call Max', archetype:'Commitment grappler', speed:3.7, jump:14.8, color:'#ffba69', victory:'Closed the position. Opened a new rivalry.' },
  ...
  mia: { id:'mia', name:'MEV Mia', archetype:'Fast pressure', speed:4.5, jump:15.6, color:'#ff86c2', victory:'First in line. Last one standing.' },
...
function movesFor(id) {
  const max=id==='max';
  return {
    light:normal('light',max?'Margin Jab':'Diamond Knuckle','Light',6,3,13,55,max?75:85,'mid',{cancels:['special','downSpecial','forwardSpecial'],counterplay:'A short check. Walk outside its reach and whiff-punish.'}),
...
export function createFight({characters=['max','diamond'],stage='dead-mall',rulesVersion=RUMBLE_RULES_VERSION}={}) {
  if(![1,RUMBLE_RULES_VERSION].includes(rulesVersion)) throw new Error('Unknown fight rules');
  ...
  return {version:rulesVersion,tick:0,phase:'intro',phaseTick:0,round:1,wins:[0,0],winner:null,stage,players:[fighter(characters[0],320,1),fighter(characters[1],680,-1)],roundTicks:3600,events:[],bounds:{left:42,right:958,warning:false,active:false},hitstop:0,draws:0};
}
...
function completeMatch(s) {
  s.phase='finished';s.phaseTick=0;
  emit(s,'finish',s.winner,s.winner===null?'MATCH DRAW':s.finisher?.activated?'FINISHER COMPLETE':'PORTFOLIO RESTORED');
}
function tickFinisher(s,inputs) {
  const slot=s.finisher.player,p=s.players[slot],edge=inputs[slot]&~p.previousInput;
  s.players.forEach((fighter,i)=>{fighter.previousInput=inputs[i];});
  if(s.phase==='finishWindow') {
    if(s.phaseTick>=FINISH_WINDOW_TICKS) {completeMatch(s);return;}
    if(edge&INPUT.SUPER) {
      s.phase='finisher';s.phaseTick=0;s.finisher.activated=true;
      emit(s,'finisherStart',slot,FINISHERS[p.character].name);
    }
  } else {
    if(s.phaseTick===FINISHER_IMPACT_TICK)emit(s,'finisherImpact',slot,FINISHERS[p.character].name);
    if(s.phaseTick>=FINISHER_DURATION_TICKS)completeMatch(s);
  }
}
function newRound(s) {
...
export function stepFight(previous,rawInputs=[0,0]) {
  ...
  if(s.phase==='finishWindow'||s.phase==='finisher'){tickFinisher(s,inputs);return s;}
  if(s.phase==='intro') {if(s.phaseTick>=90){s.phase='fight';s.phaseTick=0;emit(s,'fight',undefined,'TRADE BLOWS');}return s;}
  if(s.phase==='roundOver') {
    if(s.phaseTick>=120) {
      if(s.wins.some(n=>n>=2)||s.round>=5) {
        s.winner=s.wins[0]===s.wins[1]?null:s.wins[0]>s.wins[1]?0:1;
        s.phaseTick=0;
        if(s.version>=2&&s.winner!==null&&s.roundWinner===s.winner&&s.players[1-s.winner].hp===0&&s.players[s.winner].hp>0) {
          s.phase='finishWindow';
          s.finisher={player:s.winner,character:s.players[s.winner].character,activated:false};
          s.players.forEach((p,i)=>{p.previousInput=inputs[i];p.action=null;p.vx=0;p.vy=0;p.y=0;p.grounded=true;p.crouching=false;p.throwPending=null;});
          emit(s,'finishReady',s.winner,'FINISH IT · PRESS SUPER');
        } else completeMatch(s);
      }
      else newRound(s);
    }
    return s;
...
export function botInput(s,slot=1) {
  if(s.phase==='finishWindow')return s.finisher.player===slot&&s.phaseTick===75?INPUT.SUPER:0;
