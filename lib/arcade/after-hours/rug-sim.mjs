...
export function stepRug(s,input={}){
 ...
 const forward=Number(!!input.forward)-Number(!!input.back),side=Number(!!input.right)-Number(!!input.left),len=Math.max(1,Math.hypot(forward,side));let dx=(-Math.sin(p.yaw)*forward+Math.cos(p.yaw)*side)/len,dz=(-Math.cos(p.yaw)*forward-Math.sin(p.yaw)*side)/len;
