...
export class AfterHoursAudio{
 ...
 for(const e of s.events){if(['shot','smash','bail','hurt'].includes(e.type))this.tone(e.type==='shot'?170:90,.13,'sawtooth',.22,22);else if(['secret','bank','kill'].includes(e.type)){this.tone(440,.18,'triangle',.18);this.tone(660,.25,'triangle',.1);}else if(e.type==='land')this.tone(90,.09,'triangle',.1+(e.strength||0)*.12,35);else if(e.type==='jump')this.tone(240,.06,'triangle',.09,65);else if(e.type==='warning')this.tone(880,.2,'square',.08);else if(e.type==='trick')this.tone(220+s.combo?.mult*24,.08,'triangle',.08);}}
