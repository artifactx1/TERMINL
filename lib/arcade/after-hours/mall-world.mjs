export const SKATERS=[{id:'barry',name:'BARRY',trait:'A little more rail forgiveness.',special:'ROUND TRIP',color:'#ffad65'},{id:'paul',name:'PAPER HANDS PAUL',trait:'Safe banks pay 5% more.',special:'EARLY EXIT',color:'#e4e2bd'},{id:'max',name:'MARGIN CALL MAX',trait:'Faster meter. Bigger wipeouts.',special:'100X LEVERAGE',color:'#d179e8'},{id:'pigeon',name:'THE PIGEON',trait:'More air control. Less top speed.',special:'SHITCOIN',color:'#7bddc7'}];
...
export function makeMall(){
 ...
 const solids=[{x:-86,z:0,w:2,d:174,h:12},{x:86,z:0,w:2,d:174,h:12},{x:0,z:-86,w:174,d:2,h:12},{x:0,z:86,w:174,d:2,h:12}];
 const rails=[],props=[],ramps=[],tapes=[],labels=[];
 for(const [zi,zone]of ZONES.entries()){
  const {x,z,y}=zone;
  for(const side of [-1,1])solids.push({x:x+side*24,z:z-6,w:1,d:25,h:6,y,color:'#34454e'});
  rails.push({id:`${zone.id}-rail`,x:x-17,z:z+14,bx:x+17,bz:z+14,y:y+.8});
  ramps.push({x:x-12,z:z+5,w:7,d:6,y,rise:2.5,color:'#baa48b'},{x:x+13,z:z-12,w:6,d:5,y,rise:2.1,color:'#9e9597'});
 ...
 floors.push(...ramps);
 return {base:0,floors,solids,rails,props,tapes,labels,zones:ZONES,spawn:{x:0,z:20,y:0,yaw:0}};
}
