...
export function stepMall(s,input={}){
 ...
 if(edge('bail'))bail(s);
 p.yaw+=(Number(!!input.right)-Number(!!input.left))*t.turn*dt*(p.grounded?1:s.character==='pigeon'?1.4:1.1);
 if(edge('turn')){p.yaw+=Math.PI;trick(s,'REVERT',70);}
 const cap=s.character==='pigeon'?t.speed*.94:t.speed;
 p.speed=clamp(p.speed+(input.forward?t.acceleration:input.back?-38:-3)*dt,-8,cap);
 if(s.boost>0)s.boost--;
 if(edge('special')&&s.meter>=100){s.events.push({type:'special'});s.meter=0;trick(s,{barry:'ROUND TRIP',paul:'EARLY EXIT',max:'100X LEVERAGE',pigeon:'SHITCOIN'}[s.character],1000);if(s.character==='paul')bank(s);else if(s.character==='max')s.boost=360;else if(s.character==='barry'){p.spin+=Math.PI*2;p.vy=12;p.grounded=false;}else{p.vy=13;p.grounded=false;for(const o of s.world.props)if(!o.broken&&!o.unbreakable&&distance(p,o)<10)destroy(s,o);}}
 if(edge('jump')&&(p.grounded||p.rail)){p.vy=t.jump+(p.rail?2:0);p.grounded=false;p.rail=null;trick(s,'OLLIE',60);s.events.push({type:'jump'});}
 if(!p.grounded){p.spin+=(Number(!!input.right)-Number(!!input.left))*.09;if(edge('flip'))trick(s,input.left?'HEELFLIP':input.right?'IMPOSSIBLE':'KICKFLIP',180);if(edge('grab'))trick(s,input.left?'MELON':input.right?'MUTE':'INDY',220);}
 ...
 if(input.grind&&s.tick>15){for(const r of s.world.rails){const q=nearestRail(p,r);if(q.distance<t.railSnap+(s.character==='barry'?.4:0)&&Math.abs(p.y-r.y)<2&&q.t>.02&&q.t<.98&&Math.abs(p.speed)>3){p.rail=r.id;p.y=r.y;trick(s,'BOARDSLIDE',200);break;}}}}
 ...
 if(edge('bank')&&p.grounded)bank(s);
