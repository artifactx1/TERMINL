...
export function stepMall(s,input={}){
 ...
 p.yaw+=p.steer*t.turn*dt*(p.grounded?1:s.character==='pigeon'?.28:.16);
 ...
 if(!p.rail){
  ...
  if(input.grind&&s.tick>15&&!p.railCooldown&&p.vy<=0){for(const r of s.world.rails){const q=nearestRail(p,r);if(q.distance<t.railSnap+(s.character==='barry'?.4:0)&&Math.abs(p.y-r.y)<2&&q.t>.02&&q.t<.98&&Math.abs(p.speed)>3){p.rail=r.id;p.y=r.y;p.balance=0;p.balanceAge=0;p.visualSpin=0;trick(s,'BOARDSLIDE',200);break;}}}
 }
