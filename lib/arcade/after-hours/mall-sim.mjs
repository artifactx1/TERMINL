...
export function stepMall(s,input={}){
 ...
 if(p.jumpBuffer&&(p.grounded||p.rail||p.coyote)){p.vy=t.jump+(p.rail?2:0)+Math.max(0,p.rampSlope||0)*p.speed*.35;p.grounded=false;p.rail=null;p.railCooldown=18;p.jumpBuffer=0;p.coyote=0;p.jumpedAt=s.tick;p.spin=0;trick(s,'OLLIE',60);s.events.push({type:'jump'});}
 if(!p.grounded&&!p.rail){
  p.spin+=steering*.105;p.visualSpin=p.spin;
  ...
  if(p.grounded){p.rampSlope=travel>.001?clamp((floor-oldFloor)/travel,-1,1):0;p.y=floor;p.vy=0;}
   ...
   if(p.y<=floor){
    if(p.vy<-24)bail(s,'HEAVY LANDING');
     ...
     p.landedAt=s.tick;s.events.push({type:'land',strength:Math.min(1,Math.abs(p.vy)/14)});
    }
