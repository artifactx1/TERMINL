import {clamp,inside,nearestRail,floorAt,distance,random,moveSolid,angleDelta} from './math.mjs';
...
export function stepMall(s,input={}){
 ...
 const steering=clamp(input.steer??(Number(!!input.right)-Number(!!input.left)),-1,1);
 p.steer+=(steering-p.steer)*.24;
 // Trucks carve as the wheels roll. A stationary deck cannot yaw in place.
 p.yaw+=p.steer*t.turn*dt*(p.grounded?clamp(p.speed/6,0,1):s.character==='pigeon'?.28:.16);
 if(p.grounded&&!p.rail)p.visualSpin=angleDelta(p.visualSpin||0,0)*.76;
 if(edge('turn')&&p.grounded){p.yaw+=Math.PI;p.visualSpin=-Math.PI;trick(s,'REVERT',70);}
 p.pushing=!!input.forward&&!input.back&&p.grounded&&!p.rail;
 if(p.pushing)p.pushPhase=(p.pushPhase||0)+1;
 p.trickCooldown=Math.max(0,(p.trickCooldown||0)-1);
 ...
 p.speed=clamp(p.speed+(p.grounded||p.rail?(input.back?-38:input.forward?t.acceleration:-3):0)*dt,0,cap);
 if(s.boost>0)s.boost--;
 if(edge('special')&&s.meter>=100){s.events.push({type:'special'});s.meter=0;trick(s,{barry:'DIAMOND LINE',paul:'COLD WALLET',max:'100X LEVERAGE',pigeon:'SANDWICH ATTACK'}[s.character],1000);if(s.character==='paul')bank(s);else if(s.character==='max')s.boost=360;else if(s.character==='barry'){p.spin+=Math.PI*2;p.visualSpin+=Math.PI*2;p.vy=12;p.grounded=false;p.jumpedAt=s.tick;}else{p.vy=13;p.grounded=false;for(const o of s.world.props)if(!o.broken&&!o.unbreakable&&distance(p,o)<10)destroy(s,o);}}
 const airborneAction=edge('action')&&!p.grounded&&!p.rail;
 if(edge('jump')||(edge('action')&&!airborneAction))p.jumpBuffer=7;
 if(p.jumpBuffer&&(p.grounded||p.rail||p.coyote)){p.vy=t.jump+(p.rail?2:0)+Math.max(0,p.rampSlope||0)*p.speed*.35;p.grounded=false;p.rail=null;p.railCooldown=18;p.jumpBuffer=0;p.coyote=0;p.jumpedAt=s.tick;p.spin=0;p.trickType=null;p.trickStarted=null;trick(s,'OLLIE',60);s.events.push({type:'jump'});}
 if(!p.grounded&&!p.rail){
  // Air steering changes the travel heading. A held trick + steering requests a spin.
  if(input.flip||input.grab||(input.action&&p.trickType==='flip'))p.spin+=steering*.105;
  p.visualSpin=p.spin;
  if(!p.trickCooldown&&(edge('flip')||airborneAction||edge('grab'))){
   const grab=edge('grab');p.trickType=grab?'grab':'flip';p.trickStarted=s.tick;p.trickDuration=grab?22:26;p.trickCooldown=p.trickDuration;
   trick(s,grab?(input.left?'MELON':input.right?'MUTE':'INDY'):(input.left?'HEELFLIP':input.right?'IMPOSSIBLE':'KICKFLIP'),grab?220:180);
  }
 }
 if(p.rail&&input.grind){const r=s.world.rails.find(r=>r.id===p.rail),q=nearestRail(p,r);p.y=r.y;p.vy=0;p.grounded=false;p.x=q.x;p.z=q.z;const a=Math.atan2(r.bx-r.x,-(r.bz-r.z)),alignment=Math.cos(p.yaw-a);p.yaw=alignment>=0?a:a+Math.PI;p.speed=Math.max(10,p.speed);if(s.tick%30===0)trick(s,input.left?'NOSESLIDE':input.right?'TAILSLIDE':'50-50',110);if(q.t<=.005||q.t>=.995){p.rail=null;p.vy=3;p.jumpedAt=s.tick;}}
 ...
 p.wheelAngle=((p.wheelAngle||0)+Math.hypot(p.x-oldX,p.z-oldZ)/.11)%(Math.PI*2);
 if(!p.rail){
  const floor=floorAt(s.world,p.x,p.z),oldFloor=floorAt(s.world,oldX,oldZ),travel=Math.hypot(p.x-oldX,p.z-oldZ);
  const onSlope=s.world.floors.some(f=>f.rise&&inside({x:oldX,z:oldZ},f)&&inside(p,f));
  if(p.grounded&&!onSlope&&floor<oldFloor-.12){p.grounded=false;p.vy=Math.max(0,p.rampSlope||0)*p.speed*.9;p.jumpedAt=s.tick;p.spin=0;p.visualSpin=0;if(p.vy>2){trick(s,'RAMP TRANSFER',150);s.events.push({type:'jump'});}}
  if(p.grounded){p.rampSlope=travel>.001?clamp((floor-oldFloor)/travel,-1,1):0;p.y=floor;p.vy=0;}
  else{
   p.vy-=t.gravity*dt;p.y+=p.vy*dt;
   if(p.y<=floor){
    if(p.vy<-24)bail(s,'HEAVY LANDING');
    else{
     if(Math.abs(p.spin)>2.5)trick(s,`${Math.max(180,Math.round(Math.abs(p.spin)/Math.PI)*180)} SPIN`,250);
     p.landedAt=s.tick;p.visualSpin=0;p.trickStarted=null;p.trickType=null;s.events.push({type:'land',strength:Math.min(1,Math.abs(p.vy)/14)});
    }
    p.y=floor;p.vy=0;p.grounded=true;p.spin=0;p.trickCooldown=0;
   }
  }
  // Touch rail assistance must not grab a rolling skater and force a 90° turn.
  // Catch a rail only after jumping above it and descending toward its deck.
  if(input.grind&&!p.grounded&&s.tick>15&&!p.railCooldown&&p.vy<=0){for(const r of s.world.rails){const q=nearestRail(p,r);if(q.distance<t.railSnap+(s.character==='barry'?.4:0)&&p.y>=r.y-.15&&p.y<r.y+.65&&q.t>.02&&q.t<.98&&Math.abs(p.speed)>3){p.rail=r.id;p.y=r.y;p.balance=0;p.balanceAge=0;p.visualSpin=0;trick(s,'BOARDSLIDE',200);break;}}}
 }
 const wasManual=p.manual;p.manual=!!input.manual&&(!input.touchAssist||s.combo.count>0)&&p.grounded&&!p.rail&&p.speed>2;
 // Touch assistance keeps a line approachable; desktop players countersteer to balance.
 if(p.rail||p.manual){
  if(p.manual&&!wasManual){p.balance=0;p.balanceAge=0;}
  p.balanceAge=(p.balanceAge||0)+1;
  p.balance=clamp((p.balance||0)+(Math.sin(p.balanceAge*.025)*.005+steering*.012)*(input.touchAssist?.18:1),-1.1,1.1);
  if(input.touchAssist)p.balance*=.96;
  if(Math.abs(p.balance)>1)bail(s,'LOST BALANCE');
  else if(p.manual){if(s.tick%60===0)trick(s,'MANUAL',85);s.combo.grace=70;}
