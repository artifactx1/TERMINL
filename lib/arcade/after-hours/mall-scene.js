...
export class MallScene extends AfterHoursScene {
 ...
 animateRig(rig,s){
  ...
  board.rotation.set(0,pose.bodyYaw,0);for(const wheel of board.userData.wheels)wheel.rotation.y=s.tick*p.speed/6;board.userData.deck.rotation.set(pose.boardPitch,0,pose.boardRoll);
 ...
 animateSkater(s){
  ...
  this.canvas.dataset.skatePose=this.skater.userData.sheet.frames[this.skater.userData.pose.frame].name;this.canvas.dataset.bodyYaw=this.skater.userData.pose.bodyYaw.toFixed(3);this.canvas.dataset.boardYaw=this.skater.userData.board.rotation.y.toFixed(3);
 }
