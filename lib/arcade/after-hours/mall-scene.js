...
export class MallScene extends AfterHoursScene {
 ...
 makeSkater(id){
  ...
  const sprite=new THREE.Mesh(geometry,material),board=makeSkateboard(this,this.boardStyle);g.add(board,sprite);g.userData={sprite,board,sheet};
  return g;
 }
 ...
 updateMallCamera(s){
  ...
  // Keep the chase view in the travel frame. World-space camera lag makes the
  // deck slew sideways under a rear-view rider card, even with a shared rig.
  this.cameraYaw=p.yaw;
  ...
  if(!this.ready){this.camera.position.copy(desired);this.ready=true;}
  else this.camera.position.y+=(desired.y-this.camera.position.y)*(1-Math.exp(-10*dt));
  this.camera.position.x=desired.x;this.camera.position.z=desired.z;
