...
import {makeSkateboard,poseSkateboard} from './skateboard';
...
export class MallScene extends AfterHoursScene {
 ...
 animateRig(rig,s){
  ...
  sprite.position.copy(poseSkateboard(board,pose,p.wheelAngle||0));sprite.material.rotation=pose.bail?-1.35:pose.lean;
 ...
 animateSkater(s){
  ...
  this.canvas.dataset.boardPivot="deck-center";this.canvas.dataset.skatePose=this.skater.userData.sheet.frames[this.skater.userData.pose.frame].name;this.canvas.dataset.bodyYaw=this.skater.userData.pose.bodyYaw.toFixed(3);this.canvas.dataset.boardYaw=this.skater.userData.board.rotation.y.toFixed(3);this.canvas.dataset.boardRoll=this.skater.userData.board.userData.deck.rotation.z.toFixed(3);this.canvas.dataset.wheelAngle=(p.wheelAngle||0).toFixed(3);
 }
