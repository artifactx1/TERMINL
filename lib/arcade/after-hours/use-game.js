...
export function useGame({kind,create,step,options}){
 ...
 useEffect(()=>{
  if(!canvas.current)return;const element=canvas.current;let raf,disposed=false,last=performance.now(),acc=0,lastUI=0,lastPaint=0,savedTapes=0,recorded=false,scene,input;
  ...
  try{scene=new AfterHoursScene(canvas.current,state,{quality:settings.current.quality,reducedMotion:matchMedia('(prefers-reduced-motion: reduce)').matches});}catch(e){console.error('After-hours renderer:',e);setError('This game needs WebGL 2. Enable hardware acceleration and reload to play.');return;}
  ...
  const frame=now=>{if(disposed)return;const elapsed=Math.min(.1,(now-last)/1000);last=now;const running=modeRef.current==='playing';audio.pause(!running);audio.setVolume(settings.current.muted?0:.12);if(running){acc+=elapsed;const controls=input.read();let ticks=0;while(acc>=1/60&&ticks<6){step(state,controls);if(ticks===0){controls.lookX=controls.lookY=0;controls.weapon=null;}audio.update(state);acc-=1/60;ticks++;if(state.phase!=='playing')break;}}else acc=0;
