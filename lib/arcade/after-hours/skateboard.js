...
export function makeSkateboard(scene,style='classic'){
 ...
 if(!scene.gripMaterial){
  ...
  const map=new THREE.CanvasTexture(c);map.colorSpace=THREE.SRGBColorSpace;map.anisotropy=4;scene.disposables.push(map);scene.gripMaterial=new THREE.MeshBasicMaterial({map,transparent:true,alphaTest:.2});scene.disposables.push(scene.gripMaterial);
 }
 const grip=scene.plane(deck,0,.285,0,.77,2.15,scene.gripMaterial);grip.rotation.x=-Math.PI/2;
 const wheels=[];
 for(const z of [-.7,.7]){
  scene.mesh(deck,0,.155,z,.66,.085,.14,'#adc0ba');
  for(const x of [-.43,.43]){
   const wheel=scene.mesh(deck,x,.105,z,.11,.12,.11,'#dcd6ac','cylinder');wheel.rotation.z=Math.PI/2;wheels.push(wheel);
   scene.mesh(deck,x*1.15,.105,z,.028,.05,.05,'#7f9586','sphere');
  }
 }
 root.userData={deck,wheels};return root;
}
