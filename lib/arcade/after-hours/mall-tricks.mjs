...
export function clearAir(p){p.airMove=null;p.queuedAir=null;p.spinMove=null;p.trickType=null;p.trickStarted=null;p.trickCooldown=0;p.spin=0;p.visualSpin=0;}
...
export function stepAir(s,input,edge,steering){
 ...
 // One deliberate press in the catch window carries into the next move. Holding
 // a button never fills the queue, and a landing always discards it.
 if(p.queuedAir&&p.queuedAir.expires<s.tick)p.queuedAir=null;
 if(p.queuedAir&&!p.airMove){const next=p.queuedAir.id;p.queuedAir=null;startAir(s,next);}
 ...
 if(id){
  if(p.airMove&&s.tick-p.airMove.started>=p.airMove.duration-12)p.queuedAir={id,expires:s.tick+13};
  else startAir(s,id);
 }
}
export function landAir(s){
 ...
 // Give the player a real landing window even after a long transfer. Without
 // this, a completed aerial can bank on the very frame the wheels touch down.
 if(s.combo.count)s.combo.grace=Math.max(s.combo.grace,45);
 clearAir(p);
}
