...
export function sampleSkatePose(s,cameraYaw){
 ...
 let frame=SKATE_CLIPS.coast,bodyLift=0,boardPitch=0,boardRoll=0;
 ...
 if(p.bail){frame=SKATE_CLIPS.crouch;}
  ...
  bodyLift=trickActive?.24:.12;
  const flightAge=s.tick-(p.jumpedAt??s.tick);boardPitch=Math.sin(clamp(flightAge/18,0,1)*Math.PI)*.34;
  if(trickActive&&p.trickType==='flip'){const progress=clamp(age/duration,0,1);boardRoll=Math.PI*2*(progress*progress*(3-2*progress));bodyLift+=Math.sin(progress*Math.PI)*.25;}
 ...
 return {frame:facingFrame(bodyYaw,cameraYaw,frame),rearFrame:frame,bodyYaw,bodyLift,boardPitch,boardRoll,lean:(p.steer||0)*-.1,landing,bail:p.bail||0};
}
