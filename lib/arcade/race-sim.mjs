/** WEN LAMBO v3. Fixed 60 Hz, input-only authority. Archived v1/v2 replays remain exact. */
...
export const RACE_RULES_VERSION=3;
...
function drive(s,p,slot,mask,g){
  ...
  const requested=Number(!!(mask&2))-Number(!!(mask&1));
  p.drifting=!!(mask&16)&&signedSpeed>2&&!p.offRoad&&!!requested;
  const pace=clamp(speed/v.maxSpeed,0,1);
  // Progressive turn-in, faster countersteering and positive self-centering.
  // Limit wheel lock at racing speed while keeping full low-speed manoeuvrability.
  const lock=p.drifting?1:1-.27*pace;
  const targetSteer=requested*lock;
  const reversing=requested&&p.steer*requested<0;
  const steeringStep=requested===0?.13:reversing?.115:.08-.032*pace;
  ...
  p.yawRate=q(oldYaw+(targetYaw-oldYaw)*(requested===0||reversing?.30:.20));p.angle=q(p.angle+p.yawRate);
...
export function raceBotInput(s,slot){
  ...
  const desiredSpeed=Math.abs(difference)>.6?1.9:bend>.55?2.5:bend>.3?3.4:VEHICLES[p.vehicle].maxSpeed;
  let mask=(p.speed>desiredSpeed+.12?8:4)|(difference<-.035?1:difference>.035?2:0);
  if(bend<.12&&Math.abs(difference)<.08&&Math.abs(p.steer)<.12&&p.boost>20)mask|=32;
