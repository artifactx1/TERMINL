...
export async function fetchAllowlist(wallet) {
  ...
  try {
    const res = await fetch(`/api/allowlist/${wallet}`, { cache: "no-store", signal: AbortSignal.timeout(10_000) });
...
export function describeStage({ stage, stageClaimed = 0n, minted = 0n, lazySupply = 0n, dropEndsAt = 0n, now }) {
  ...
  /* An uncapped stage on a drop whose total we could not read has no known
   * supply bound. The picker can still use the wallet's proven allocation. */
