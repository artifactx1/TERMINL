...
import { validateMoonReplay, replayMoon } from "./moon-mission.mjs";
...
export function decodeChallenge(code) {
  try {
    ...
    return validateChallenge({ v:p[0], name:p[1], coin:p[2], seed:p[3], endTick:p[4], moves:p[5].map(([tick,value]) => p[0] === 3 ? ({tick,input:value}) : ({tick,lane:value})) });
  } catch { return null; }
}
