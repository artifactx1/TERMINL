...
/**
 * The backend both allowlist routes talk to, or "" when none is known for this
 * chain (which reads as a public-only drop rather than as an error).
 *
 * ALLOWLIST_API_URL is deliberately NOT prefixed NEXT_PUBLIC_, and is read on
 * the server only — same reasoning as RPC_URL in lib/mint.js. Nothing in the
 * browser ever calls the backend directly: its CORS admits artifactx.app alone,
 * and a request with no Origin header — which is what a Next API route sends —
 * is what passes.
 */
export const allowlistBase = () => {
  const override = typeof window === "undefined" ? (process.env.ALLOWLIST_API_URL || "").trim() : "";
  return (override || ALLOWLIST_HOSTS[CHAIN.id] || "").replace(/\/+$/, "");
};
...
export async function fetchPhases() {
  try {
    const res = await fetch("/api/phases", { cache: "no-store", signal: AbortSignal.timeout(10_000) });
    if (!res.ok) throw new Error(`phases route ${res.status}`);
    return await res.json();
  } catch {
    return { published: false, stages: [], failed: true };
  }
}
