...
try{
  ...
  for(let index=0;index<6;index++){
    ...
    for(let batch=0;batch<190;batch++){
      await page.evaluate(()=>{const c=document.querySelector('canvas');for(let i=0;i<90;i++){const tick=Number(c.dataset.tick),action=window.fightMoves[window.fightMoveIndex];if(action?.tick===tick){for(const [bit,key]of [[1,'ArrowLeft'],[2,'ArrowRight'],[4,'ArrowUp'],[8,'ArrowDown'],[16,'j'],[32,'k'],[64,'l'],[128,' '],[256,'u'],[512,'i'],[1024,'o']])if(!!(window.fightMask&bit)!==!!(action.input&bit))window.dispatchEvent(new KeyboardEvent(action.input&bit?'keydown':'keyup',{key,bubbles:true,cancelable:true}));window.fightMask=action.input;window.fightMoveIndex++;}window.skipFightPaint=i<89;window.advanceFightFrame();window.skipFightPaint=false;if(c.dataset.phase==='finished')break;}});
