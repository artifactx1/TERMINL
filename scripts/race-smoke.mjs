...
try{
  await page.goto((process.env.ARCADE_TEST_URL||'http://localhost:4000')+'/os/lambo');await page.getByRole('button',{name:'PRACTICE CUP →',exact:true}).click();
  ...
  await page.getByRole('button',{name:'← LEAVE',exact:true}).click();await page.getByRole('button',{name:/Sunset Canyon route preview/}).click();await page.getByRole('button',{name:/Spectre RX arcade roadster with Pepe/}).click();await page.getByRole('button',{name:'LEARN TO DRIVE ↗',exact:true}).click();
