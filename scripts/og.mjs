...
async function card() {
  ...
  const text = Buffer.from(`<svg width="${W}" height="${H}">
    ...
    <text x="56" y="232" font-family="monospace" font-size="52" font-weight="700" fill="#ffffff">You said</text>
    <text x="56" y="286" font-family="monospace" font-size="52" font-weight="700" fill="#ffffff">you were</text>
    <text x="56" y="340" font-family="monospace" font-size="52" font-weight="700" fill="#ffffff">done.</text>
    <text x="56" y="394" font-family="monospace" font-size="20" fill="#9aa89f">${esc(
      `${site.supply.toLocaleString('en-US')} machines. Familiar bad habits.`,
    )}</text>
    <text x="56" y="426" font-family="monospace" font-size="20" fill="#7dff5c" letter-spacing="2">${esc(
      'MEET THE DEGENS. PLAY THE ARCADE.',
    )}</text>
  </svg>`);
