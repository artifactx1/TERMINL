...
test('content capabilities, expanded fighters and race rooms enforce game-specific schemas and replay rules',async(t)=>{
  ...
  const health=await fetch(`${runtime.http}/health`).then(r=>r.json());assert.equal(health.characters.length,6);assert.deepEqual(health.games,['rekt-rumble','wen-lambo']);assert.equal(health.vehicles.length,5);
  ...
  d.send({type:'join',game:'wen-lambo',rulesVersion:3,name:'Driver B',character:'glacier',code:invite.code,token:invite.token});await d.wait(m=>m.type==='joined');
