...
const moduleURL=`data:text/javascript;base64,${Buffer.from(await readFile(new URL('../lib/arcade/race-sim.mjs',import.meta.url),'utf8')).toString('base64')}`;
...
try{
  ...
  await Promise.all(pages.map(p=>p.waitForFunction(()=>window.__result?.status==='completed',null,{timeout:300000})));
  const results=await Promise.all(pages.map(p=>p.evaluate(()=>window.__result)));assert.deepEqual(results[0],results[1]);assert.deepEqual(results[0],results[2]);assert.equal(results[0].reason,'match_complete');assert.equal(results[0].game,'wen-lambo');assert.equal(results[0].races.length,2);
  assert.ok(results[0].races.every(r=>r.times.every(n=>n!==null)),'Both drivers must finish both tracks through ordinary input');
  ...
  await click(a,'← LEAVE');for(let i=0;i<4;i++){await click(a,'PRACTICE CUP →');await click(a,'← LEAVE');}
